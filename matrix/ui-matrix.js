...
/*     	"disabled": [1, 2, 3, 4], */
	...
	onMatrix: function (id, panel) {
      ...
      jTConfig.matrix.groups = function (miniset, kit) {
        var groups = {
  		...
  		self.matrixKit.query(jT.settings.baseUrl + "/substanceowner/NWKI-9F4E86D0-C85D-3E83-8249-A856659087DA/dataset");
		}
	},
	...
	onEndpoint: function (id, panel) {
      ...
      self.substanceKit.query(jT.settings.baseUrl + "/substance?type=related&compound_uri=" + encodeURIComponent("https://apps.ideaconsult.net/enanomapper/compound/85"));
	  }
	  else {// i.e. endpoints
  	  if (sub.firstElementChild == null) {
    	  var root = document.createElement('div');
