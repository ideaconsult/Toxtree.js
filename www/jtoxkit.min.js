...
var jToxDataset=(function(){var defaultSettings={"showTabs":true,"showExport":true,"showControls":true,"pageSize":20,"pageStart":0,"metricFeature":"http://www.opentox.org/api/1.1#Similarity","fnAccumulate":function(fId,oldVal,newVal){if(ccLib.isNull(newVal))
...
return ccLib.getJsonValue(data,$.isArray(feature.accumulate)?feature.accumulate[0]:feature.accumulate);else
...
accArr=[accArr];for(var v=0;v<accArr.length;++v){var oldVal=ccLib.getJsonValue(entry,accArr[v]);var newVal=entry.values[fid];if(!ccLib.isNull(fnValue))
ccLib.setJsonValue(entry,accArr[v],ccLib.fireCallback(fnValue,null,fid,oldVal,newVal));else if(!$.isArray(oldVal))
...
base=baseFeatures;features=$.extend(features,bases);for(var fid in features){var sameAs=cls.findSameAs(fid,features);features[fid]=$.extend(features[fid],features[sameAs],{originalId:fid});}
