...
$(el).attr($(el).data('attribute'),value);else
...
return arr;},"Other":function(name,miniset){var arr=[];for(var f in miniset.features){var sameAs=jToxDataset.findSameAs(f,miniset.features);if(!miniset.features[f].used&&!miniset.features[sameAs].used){arr.push(f);miniset.features[sameAs].used=true;}}
return arr;}},"exports":[{type:"chemical/x-mdl-sdfile",icon:"images/sdf.jpg"},{type:"chemical/x-cml",icon:"images/cml.jpg"},{type:"chemical/x-daylight-smiles",icon:"images/smi.png"},{type:"chemical/x-inchi",icon:"images/inchi.png"},{type:"text/uri-list",icon:"images/link.png"},{type:"application/pdf",icon:"images/pdf.png"},{type:"text/csv",icon:"images/excel.png"},{type:"text/plain",icon:"images/excel.png"},{type:"text/x-arff",icon:"images/weka.png"},{type:"text/x-arff-3col",icon:"images/weka.png"},{type:"application/rdf+xml",icon:"images/rdf.gif"},{type:"application/json",icon:"images/json.png"}],"baseFeatures":{"http://www.opentox.org/api/1.1#Diagram":{title:"Diagram",search:false,used:true,process:function(entry){entry.compound.diagramUri=entry.compound.URI.replace(/(.+)(\/conformer.*)/,"$1")+"?media=image/png";},render:function(col,fId){col["mData"]="compound.diagramUri";col["mRender"]=function(data,type,full){return(type!="display")?"-":'<a target="_blank" href="'+full.compound.URI+'"><img src="'+data+'" class="jtox-ds-smalldiagram"/></a>';};col["sClass"]="paddingless";col["sWidth"]="125px";return col;}},"http://www.opentox.org/api/1.1#Similarity":{title:"Similarity",location:"compound.metric",search:true,used:true},}}};var baseFeatures={"http://www.opentox.org/api/1.1#REACHRegistrationDate":{title:"REACH Date",location:"compound.reachdate",used:true,accumulate:true},"http://www.opentox.org/api/1.1#CASRN":{title:"CAS",location:"compound.cas",used:true,accumulate:true},"http://www.opentox.org/api/1.1#ChemicalName":{title:"Name",location:"compound.name",used:true,accumulate:true},"http://www.opentox.org/api/1.1#TradeName":{title:"Trade Name",location:"compound.tradename",used:true,accumulate:true},"http://www.opentox.org/api/1.1#IUPACName":{title:"IUPAC Name",location:["compound.name","compound.iupac"],used:true,accumulate:true},"http://www.opentox.org/api/1.1#EINECS":{title:"EINECS",location:"compound.einecs",used:true,accumulate:true},"http://www.opentox.org/api/1.1#InChI":{title:"InChI",location:"compound.inchi",used:true,shorten:true,accumulate:true},"http://www.opentox.org/api/1.1#InChI_std":{title:"InChI",location:"compound.inchi",used:true,shorten:true,accumulate:true},"http://www.opentox.org/api/1.1#InChIKey":{title:"InChI Key",location:"compound.inchikey",used:true,accumulate:true},"http://www.opentox.org/api/1.1#InChIKey_std":{title:"InChI Key",location:"compound.inchikey",used:true,accumulate:true},"http://www.opentox.org/api/1.1#InChI_AuxInfo":{title:"InChI Aux",location:"compound.inchi",used:true,accumulate:true},"http://www.opentox.org/api/1.1#InChI_AuxInfo_std":{title:"InChI Aux",location:"compound.inchi",used:true,accumulate:true},"http://www.opentox.org/api/1.1#IUCLID5_UUID":{title:"IUCLID5 UUID",location:"compound.i5uuid",used:true,shorten:true,accumulate:true},"http://www.opentox.org/api/1.1#SMILES":{title:"SMILES",location:"compound.smiles",used:true,shorten:true,accumulate:true},"http://www.opentox.org/api/dblinks#CMS":{title:"CMS",used:true,accumulate:true},"http://www.opentox.org/api/dblinks#ChEBI":{title:"ChEBI",used:true,accumulate:true},"http://www.opentox.org/api/dblinks#Pubchem":{title:"PubChem",used:true,accumulate:true},"http://www.opentox.org/api/dblinks#ChemSpider":{title:"ChemSpider",used:true,accumulate:true},"http://www.opentox.org/api/dblinks#ChEMBL":{title:"ChEMBL",used:true,accumulate:true},"http://www.opentox.org/api/dblinks#ToxbankWiki":{title:"Toxbank Wiki",used:true,accumulate:true},};var instanceCount=0;var cls=function(root,settings){var self=this;self.rootElement=root;$(root).addClass('jtox-toolkit');var newDefs=$.extend(true,{"configuration":{"baseFeatures":baseFeatures}},defaultSettings);self.settings=$.extend(true,{},newDefs,jToxKit.settings,settings);self.features=null;self.dataset=null;self.groups=null;self.fixTable=self.varTable=null;self.instanceNo=instanceCount++;self.entriesCount=null;self.suspendEqualization=false;self.orderList=[];root.appendChild(jToxKit.getTemplate('#jtox-dataset'));var pane=$('.jtox-ds-control',self.rootElement)[0];if(self.settings.showControls){ccLib.fillTree(pane,{"pagesize":self.settings.pageSize});$('.next-field',pane).on('click',function(){self.nextPage();});$('.prev-field',pane).on('click',function(){self.prevPage();});$('select',pane).on('change',function(){self.queryEntries(self.settings.pageStart,parseInt($(this).val()));})
...
divEl.id=tabId;all.appendChild(divEl);divEl=$('.jtox-exportlist',divEl)[0];for(var i=0,elen=self.settings.configuration.exports.length;i<elen;++i){var expo=self.settings.configuration.exports[i];var el=jToxKit.getTemplate('#jtox-ds-download');divEl.appendChild(el);$('a',el)[0].href=ccLib.addParameter(self.datasetUri,"media="+encodeURIComponent(expo.type));var img=el.getElementsByTagName('img')[0];img.alt=img.title=expo.type;img.src=self.baseUrl+expo.icon;}}
...
return ccLib.getJsonValue(data,$.isArray(feature.location)?feature.location[0]:feature.location);else
...
if(rlen>0)
$(self.fixTable).dataTable().fnSort([[1,"asc"]]);},"oLanguage":{"sEmptyTable":" - "}}))[0];},updateTables:function(){var self=this;self.filterEntries($('.jtox-ds-control input',self.rootElement).val());},prepareGroups:function(miniset){var self=this;var grps=self.settings.configuration.groups;self.groups={};for(var i in grps){var grp=grps[i];if(ccLib.isNull(grp))
...
self.entriesCount=qStart+qSize;self.dataset=cls.processDataset(dataset,self.features,self.settings.fnAccumulate,self.settings.pageStart);self.updateTables();if(self.settings.showControls){var pane=$('.jtox-ds-control',self.rootElement)[0];ccLib.fillTree(pane,{"pagestart":qStart+1,"pageend":qStart+qSize,});var nextBut=$('.next-field',pane);if(self.entriesCount===null||qStart+qSize<self.entriesCount)
...
self.prepareTables();self.equalizeTables();self.queryEntries(self.settings.pageStart,self.settings.pageSize);}});},};cls.shortFeatureId=function(fId){return fId.substr(fId.indexOf('#')+1);};cls.processEntry=function(entry,features,fnValue){for(var fid in features){var feature=features[fid];var newVal=entry.values[fid];if(!!feature.accumulate&&newVal!==undefined&&feature.location!==undefined){var accArr=feature.location;if(!$.isArray(accArr))
...
bases=baseFeatures;features=$.extend(features,bases);for(var fid in features){var sameAs=cls.findSameAs(fid,features);features[fid]=$.extend(features[fid],features[sameAs],{originalId:fid});}
...
new jToxDataset(element,params);};$('.jtox-toolkit').each(function(i){var dataParams=$.extend(true,self.settings,$(this).data());if(!dataParams.manualInit){var el=this;if(!ccLib.isNull(dataParams.configFile)){self.call({settings:dataParams},dataParams.configFile,function(config){if(!!config)
...
self.templateRoot=root;},getTemplate:function(selector){var el=$(selector,this.templateRoot)[0];if(!!el){var el=el.cloneNode(true);el.removeAttribute('id');}
