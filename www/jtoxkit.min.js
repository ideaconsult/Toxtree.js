...
divEl.id=tabId;all.appendChild(divEl);divEl=jT.$('.jtox-exportlist',divEl)[0];for(var i=0,elen=self.settings.configuration.exports.length;i<elen;++i){var expo=self.settings.configuration.exports[i];var el=jT.getTemplate('#jtox-ds-download');divEl.appendChild(el);jT.$('a',el)[0].href=ccLib.addParameter(self.datasetUri,"media="+encodeURIComponent(expo.type));var img=el.getElementsByTagName('img')[0];img.alt=img.title=expo.type;img.src=(jT.settings.baseUrl||self.baseUrl)+expo.icon;}}
...
substance["IUCFlags"]=flags;ccLib.fillTree(self.rootElement,substance);jT.call(self,substance.referenceSubstance.uri,function(dataset){if(!!dataset){jToxDataset.processDataset(dataset,null,fnDatasetValue);ccLib.fillTree(rootTab,dataset.dataEntry[0]);}});self.querySummary(substance.URI+"/studysummary");self.queryComposition(substance.URI+"/composition");}});}};return cls;})();window.jT=window.jToxKit={templateRoot:null,queries:{taskPoll:{method:'GET',service:"/task/{id}"},},templates:{},tools:{},settings:{jsonp:false,crossDomain:false,host:null,timeout:15000,pollDelay:200,onConnect:function(s){},onSuccess:function(s,c,m){},onError:function(s,c,m){if(!!console&&!!console.log)console.log("jToxKit call error ("+c+"): "+m+" from request: ["+s+"]");},},formBaseUrl:function(url){return!!url.host?url.protocol+"://"+url.host+(url.port.length>0?":"+url.port:'')+'/'+url.segments[0]+'/':null;},initKit:function(element){var self=this;var dataParams=self.$.extend(true,self.settings,self.$(element).data());if(!dataParams.manualInit){var kit=dataParams.kit;var realInit=function(params){if(!kit)
return null;if(!window[kit]&&kit.indexOf('jTox')!=0)
kit='jTox'+kit.charAt(0).toUpperCase()+kit.slice(1);var fn=window[kit];if(typeof fn=='function')
return new fn(element,params);else if(typeof fn=="object"&&typeof fn.init=="function")
return fn.init(element,params);return null;};if(!ccLib.isNull(dataParams.configFile)){self.call({settings:dataParams},dataParams.configFile,function(config){if(!!config)
dataParams['configuration']=self.$.extend(true,dataParams['configuration'],config);realInit(dataParams);});}
else
realInit(dataParams);}},init:function(root){var self=this;if(!root){self.initTemplates();jT.$(document).on('click','.jtox-toolkit span.ui-icon-copy',function(e){ccLib.copyToClipboard(jT.$(this).data('uuid'));return false;});var url=ccLib.parseURL(document.location);var queryParams=url.params;if(!queryParams.baseUrl)
queryParams.baseUrl=self.formBaseUrl(url);self.settings=self.$.extend(self.settings,queryParams);root=document;}
self.$('.jtox-toolkit',root).each(function(i){self.initKit(this);});},initTemplates:function(){var self=this;var root=jT.$('.jtox-template')[0];if(root===undefined){var html='';for(var t in self.templates){html+=self.templates[t];}
...
var settings=jT.$.extend({},this.settings,params);if(kit==null)
kit=this;else
