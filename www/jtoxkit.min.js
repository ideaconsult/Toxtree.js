...
obj.innerHTML=value;},setJsonValue:function(json,field,val){if(field!==undefined){try{eval("json."+field+" = val");}
...
return format;},copyToClipboard:function(text,prompt){if(!prompt){prompt="Press Ctrl-C (Command-C) to copy and then Enter.";}
window.prompt(prompt,text);},parseURL:function(url){var a=document.createElement('a');a.href=url;return{source:url,protocol:a.protocol.replace(':',''),host:a.hostname,port:a.port,query:a.search,params:(function(){var ret={},seg=a.search.replace(/^\?/,'').split('&'),len=seg.length,i=0,s;for(;i<len;i++){if(!seg[i]){continue;}
...
var jToxDataset=(function(){var defaultSettings={};var instanceCount=0;var baseFeatures={"http://www.opentox.org/api/1.1#REACHRegistrationDate":{title:"REACH Date",accumulate:"compound.reachdate"},"http://www.opentox.org/api/1.1#CASRN":{title:"CAS",accumulate:"compound.cas"},"http://www.opentox.org/api/1.1#ChemicalName":{title:"Name",accumulate:"compound.name"},"http://www.opentox.org/api/1.1#TradeName":{title:"Trade Name",accumulate:["compound.name","compound.tradename"]},"http://www.opentox.org/api/1.1#IUPACName":{title:"IUPAC Name",accumulate:["compound.name","compound.iupac"]},"http://www.opentox.org/api/1.1#EINECS":{title:"EINECS",accumulate:"compound.einecs"},"http://www.opentox.org/api/1.1#InChI":{title:"InChI",accumulate:"compound.inchi"},"http://www.opentox.org/api/1.1#InChI_std":{title:"InChI",accumulate:"compound.inchi"},"http://www.opentox.org/api/1.1#InChIKey":{title:"InChI Key",accumulate:"compound.inchikey"},"http://www.opentox.org/api/1.1#InChIKey_std":{title:"InChI Key",accumulate:"compound.inchikey"},"http://www.opentox.org/api/1.1#InChI_AuxInfo":{title:"InChI Aux",accumulate:"compound.inchi"},"http://www.opentox.org/api/1.1#InChI_AuxInfo_std":{title:"InChI Aux",accumulate:"compound.inchi"},"http://www.opentox.org/api/1.1#IUCLID5_UUID":{title:"IUCLID5 UUID",accumulate:"compound.i5uuid"},"http://www.opentox.org/api/1.1#SMILES":{title:"SMILES",accumulate:"compound.smiles"},"http://www.opentox.org/api/dblinks#CMS":{title:"CMS"},"http://www.opentox.org/api/dblinks#ChEBI":{title:"ChEBI"},"http://www.opentox.org/api/dblinks#Pubchem":{title:"Public Chem"},"http://www.opentox.org/api/dblinks#ChemSpider":{title:"Chem Spider"},"http://www.opentox.org/api/dblinks#ChEMBL":{title:"ChEMBL"},"http://www.opentox.org/api/dblinks#ToxbankWiki":{title:"Toxban Wiki"}};var commonFeatures={"diagram":{"column":0,"title":"Diagram","group":"Common","dataLocation":"compound.URI"},"cas":{"column":1,"title":"CAS","group":"Common","dataLocation":"compound.cas"},"einecs":{"colun":2,"title":"EINECS","group":"Common","dataLocation":"compound.einecs"},"name":{"column":3,"title":"Name","group":"Common","dataLocation":"compound.name"},"SMILES":{"column":4,"title":"Smiles","units":"","group":"Common","dataLocation":"compound.smiles"},"InChI":{"column":5,"title":"InChI","units":"","group":"Common","dataLocation":"compound.inchi"},"reachdate":{"column":6,"title":"REACHRegistration","units":"","group":"Common","dataLocation":"compound.reachdate"}};var cls=function(root,settings){var self=this;self.rootElement=root;self.settings=$.extend({},defaultSettings,jToxKit.settings,settings);self.features=null;self.featureGroups={"Common":[]};self.featureOrder=["Common"];instanceCount++;};cls.prototype={prepareTabs:function(root,entry){var self=this;var fr=document.createDocumentFragment();var all=document.createElement('div');fr.appendChild(all);ulEl=document.createElement('ul');all.appendChild(ulEl);var featNames={};for(var gr in self.featureGroups){var grId="jtox-ds-"+gr+"-"+instanceCount;var liEl=document.createElement('li');ulEl.appendChild(liEl);var aEl=document.createElement('a');aEl.href="#"+grId;aEl.innerHTML=gr.replace(/_/g," ");liEl.appendChild(aEl);var divEl=document.createElement('div');divEl.id=grId;for(var i=0;i<self.featureGroups[gr].length;++i){var feat=self.featureGroups[gr][i];var fEl=jToxKit.getTemplate('#jtox-ds-feature');featNames[feat]=self.features[feat].title;fEl.setAttribute('data-feature',feat);divEl.appendChild(fEl);}
all.appendChild(divEl);}
root.appendChild(fr);$('.jtox-ds-feature',all).each(function(){var fId=$(this).data('feature');$('.jtox-title',this).html(featNames[fId]);});return $(all).tabs();},prepareTables:function(){var fixedCols=[];var varCols=[];for(var i=0,glen=self.featureOrder.length;i<glen;++i){var grp=self.featureOrder[i];var gfull=self.featureGroups[grp];for(var j=0,flen=gfull.length;j<flen;++j){var f=gfull[j];var ffull=self.features[f];var col={"sTitle":ffull.title+ffull.units,"sDefaultContent":"?","mData":ffull.dataLocation!==undefined?ffull.dataLocation:"values"};if(ffull.dataLocation===undefined){col["mRender"]=(function(fid){return function(data,type,full){return data[fid];};})(f);}
if(grp=="common")
fixedCols.push(col);else
varCols.push(col);}}
var fixTable=null;var varTable=null;},groupFeatures:function(features){var self=this;self.features=features;if(!!features){for(var f in features){var fullf=features[f];var gr=cls.fixSameAs(fullf.sameAs);if(commonFeatures[gr]===undefined){gr=gr.replace(/\s/g,'_');if(self.featureGroups[gr]===undefined){self.featureOrder.push(gr);self.featureGroups[gr]=[];}
self.featureGroups[gr].push(f);}
fullf.group=gr;}
for(var f in commonFeatures)
self.featureGroups["Common"].push(f);$.extend(self.features,commonFeatures);}},clearDataset:function(){var self=this;},queryDataset:function(datasetUri){var self=this;self.clearDataset();jToxKit.call(self,datasetUri+'/feature',function(feature){if(!!feature){self.groupFeatures(feature.feature);self.prepareTabs($('.jtox-ds-features',self.rootElement)[0]);self.prepareTables();jToxKit.call(self,datasetUri,function(dataset){if(!!dataset){cls.processDataset(dataset);$('.jtox-ds-fixed table',self.rootElement).dataTable().fnAddData(dataset.dataEntry);$('.jtox-ds-variable table',self.rootElement).dataTable().fnAddData(dataset.dataEntry);}});}});},};cls.processEntry=function(entry,features,fnValue){for(var fid in entry.values){var feature=features[fid];if(feature.accumulate!==undefined){var accArr=feature.accumulate;if(!$.isArray(accArr))
accArr=[accArr];for(var v=0;v<accArr.length;++v){var oldVal=ccLib.getJsonValue(entry,accArr[v]);var newVal=entry.values[fid];if(typeof fnValue!==undefined)
ccLib.setJsonValue(entry,accArr[v],fnValue(oldVal,newVal));else if(!$.isArray(oldVal))
ccLib.setJsonValue(entry,accArr[v],oldVal+newVal);else
oldVal.push(newVal);}}}
return entry;};cls.processFeatures=function(features){features=$.extend(features,baseFeatures);for(var fid in features){var feature=features[fid];for(;;){if(feature.sameAs===undefined||feature.sameAs==null)
break;if(features[feature.sameAs]!==undefined)
feature=features[feature.sameAs];else{var base=fid.replace(/(http.+\/feature\/).*/g,"$1");if(features[base+feature.sameAs]!==undefined)
feature=features[base+feature.sameAs];else
break;}}
features[fid]=$.extend(features[fid],feature);}
return features;};cls.processDataset=function(dataset,fnValue){cls.processFeatures(dataset.feature);for(var i=0,dl=dataset.dataEntry.length;i<dl;++i){cls.processEntry(dataset.dataEntry[i],dataset.feature,fnValue);}};return cls;})();var jToxStudy=(function(){var defaultSettings={};var instanceCount=0;$(document).on('click','.jtox-toolkit span.ui-icon-copy',function(e){ccLib.copyToClipboard($(this).data('uuid'),"Press Ctrl-C (Command-C) to copy UUID:");return false;});var fnDatasetValue=function(old,value){return ccLib.extendArray(old,value!=null?value.trim().toLowerCase().split("|"):[value]).filter(ccNonEmptyFilter);};var cls=function(root,settings){var self=this;self.rootElement=root;self.suffix='_'+instanceCount++;self.settings=$.extend({},defaultSettings,jToxKit.settings,settings);var tree=jToxKit.getTemplate('#jtox-studies');root.appendChild(tree);jToxKit.changeTabsIds(tree,self.suffix);var loadPanel=function(panel){if(panel){$('.jtox-study.unloaded',panel).each(function(i){var table=this;jToxKit.call(self,$(table).data('jtox-uri'),function(study){$(table).removeClass('unloaded folded');$(table).addClass('loaded');self.processStudies(panel,study.study,false);$('.dataTable',table).dataTable().fnAdjustColumnSizing();});});}};$(tree).tabs({"select":function(event,ui){loadPanel(ui.panel);},"beforeActivate":function(event,ui){if(ui.newPanel)
...
count=0;return str.replace(/(.+)\s\(([0-9]+)\)/,"$1 ("+count+")");},ensureTable:function(tab,study){var self=this;var theTable=$('.'+study.protocol.category.code+' .jtox-study-table')[0];if(!$(theTable).hasClass('dataTable')){var colDefs=[{"sClass":"center","sWidth":"20%","mData":"protocol.endpoint"}];var headerRow=theTable.getElementsByClassName('jtox-header')[0];var before=headerRow.firstElementChild;var parCount=0;var putAGroup=function(group,fProcess){var count=0;for(var p in group){var val=fProcess(p);if(val===undefined)
...
return count;}
var formatLoHigh=function(data,type){var out="";data=data.result;if(data.loValue!==undefined&&data.upValue!==undefined){out+=(data.loQualifier==">=")?"[":"(";out+=data.loValue+", "+data.upValue;out+=(data.upQualifier=="<=")?"]":") ";}
else
...
if(!!data.unit)
out+=data.unit;return out.replace(/ /g,"&nbsp;");};var formatUnits=function(data,unit){return data!==undefined?(data+((unit!==undefined)?"&nbsp;"+unit:"")):"-";};parCount+=putAGroup(study.parameters,function(p){if(study.effects[0].conditions[p]!==undefined||study.effects[0].conditions[p+" unit"]!==undefined)
return undefined;var rFn=study.parameters[p].loValue===undefined?function(data,type,full){return formatUnits(data,full[p+" unit"]);}:function(data,type,full){return formatLoHigh(data,type);};return{"sClass":"center middle","mData":"parameters."+p,"mRender":rFn};});parCount+=putAGroup(study.effects[0].conditions,function(c){var rnFn=study.effects[0].conditions[c].loValue===undefined?function(data,type){return formatUnits(data.conditions[c],data.conditions[c+" unit"]);}:formatLoHigh;return study.effects[0].conditions[c+" unit"]===undefined?{"sClass":"center middle jtox-multi","mData":"effects","mRender":function(data,type,full){return self.renderMulti(data,type,full,rnFn);}}:undefined;});var preheaderCell=theTable.getElementsByClassName('jtox-preheader')[0].firstElementChild.nextElementSibling;if(parCount>0)
preheaderCell.setAttribute('colspan',parCount);else
preheaderCell.parentNode.removeChild(preheaderCell);colDefs.push({"sClass":"center middle jtox-multi","sWidth":"15%","mData":"effects","mRender":function(data,type,full){return self.renderMulti(data,type,full,"endpoint");}},{"sClass":"center middle jtox-multi","sWidth":"15%","mData":"effects","mRender":function(data,type,full){return self.renderMulti(data,type,full,formatLoHigh)}});before=before.nextElementSibling.nextElementSibling;parCount=putAGroup(study.interpretation,function(i){return{"sClass":"center middle jtox-multi","mData":"interpretation."+i,"sDefaultContent":"-"};});preheaderCell=preheaderCell.nextElementSibling.nextElementSibling;if(parCount>0)
preheaderCell.setAttribute('colspan',parCount);else
preheaderCell.parentNode.removeChild(preheaderCell);colDefs.push({"sClass":"center","sWidth":"15%","mData":"protocol.guidance","mRender":"[,]","sDefaultContent":"?"},{"sClass":"center","sWidth":"50px","mData":"owner.company.name","mRender":function(data,type,full){return type!="display"?''+data:'<div class="shortened">'+data+'</div>';}},{"sClass":"center","sWidth":"50px","mData":"uuid","bSearchable":false,"mRender":function(data,type,full){return type!="display"?''+data:'<div class="shortened">'+data+'</div><span class="ui-icon ui-icon-copy" title="Press to copy the UUID in the clipboard" data-uuid="'+data+'"></span>';}});$(theTable).dataTable({"bPaginate":true,"bProcessing":true,"bLengthChange":false,"bAutoWidth":false,"sDom":"rt<Fip>","aoColumns":colDefs,"fnInfoCallback":function(oSettings,iStart,iEnd,iMax,iTotal,sPre){var el=$('.jtox-study-title .data-field',$(this).parents('.jtox-study'))[0];el.innerHTML=self.updateCount(el.innerHTML,iTotal);return sPre;},"oLanguage":{"sProcessing":"<img src='"+self.baseUrl+"images/24x24_ambit.gif' border='0'>","sLoadingRecords":"No studies found.","sZeroRecords":"No studies found.","sEmptyTable":"No studies available.","sInfo":"Showing _TOTAL_ study(s) (_START_ to _END_)","sLengthMenu":'Display <select>'+
...
continue;ccLib.fillTree(aStudy,{title:onec[0].protocol.category.title+" (0)"});var theTable=self.ensureTable(tab,onec[0]);$(theTable).dataTable().fnAddData(onec);}
...
self.processComposition(composition);});},querySubstance:function(substanceURI){var self=this;self.baseUrl=jToxKit.grabBaseUrl(substanceURI,'substance');var rootTab=$('.jtox-substance',self.rootElement)[0];jToxKit.call(self,substanceURI,function(substance){if(!!substance&&!!substance.substance&&substance.substance.length>0){substance=substance.substance[0];ccLib.fillTree(rootTab,substance);jToxKit.call(self,substance.referenceSubstance.uri,function(dataset){if(!!dataset){jToxDataset.processDataset(dataset,fnDatasetValue);ccLib.fillTree(rootTab,dataset.dataEntry[0]);}});self.querySummary(substance.URI);self.queryComposition(substance.URI);}});}};return cls;})();window.jToxKit={templateRoot:null,queries:{taskPoll:"/task/<1>",},templates:{},settings:{jsonp:false,baseUrl:null,timeout:15000,pollDelay:200,onConnect:function(s){},onSuccess:function(c,m){},onError:function(c,m){},},init:function(){var self=this;self.initTemplates();var url=ccLib.parseURL(document.location);var queryParams=url.params;queryParams.host=url.host;self.settings=$.extend(self.settings,queryParams);if(!self.settings.baseUrl)
...
return el;},changeTabsIds:function(root,suffix){$('ul li a',root).each(function(){var id=$(this).attr('href').substr(1);var el=document.getElementById(id);id+=suffix;el.id=id;$(this).attr('href','#'+id);})},pollTask:function(task,callback){var self=this;if(task===undefined||task.task===undefined||task.task.length<1){ccLib.fireCallback(self.settings.onError,self,'-1',localMessage.taskFailed);return;}
...
"	    </div>"+
"	  </div>"+
"";jToxKit.templates['dataset-details']="    <div id=\"jtox-details\" class=\"jtox-details\">"+
"    </div>"+
"";jToxKit.templates['dataset-one-feature']="    <div id=\"jtox-ds-feature\" class=\"jtox-ds-feature\"><input type=\"checkbox\" checked=\"yes\" class=\"jtox-checkbox\" /><span class=\"jtox-title\"> ? </span></div>"+
