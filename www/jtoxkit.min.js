...
query=kit;});return query;};return cls;})();var jToxSearch=(function(){var defaultSettings={configuration:{handlers:{onKetcher:function(service,method,async,parameters,onready){if(service=="knocknock")
onready("You are welcome!",null);else
jT.call(null,'molecules/'+service,parameters,function(res,jhr){onready(res,jhr);});}}}};var queries={'auto':"/ui/query/compound/search/all",'similarity':"/ui/query/similarity",'smarts':"/ui/query/smarts"};var cls=function(root,settings){var self=this;self.rootElement=root;jT.$(root).addClass('jtox-toolkit');self.settings=jT.$.extend({},defaultSettings,jT.settings,settings);self.rootElement.appendChild(jT.getTemplate('#jtox-search'));self.queryKit=jToxQuery.queryKit(self.rootElement);self.queryKit.initHandlers(root);self.search={mol:"",type:""};var form=jT.$('form',self.rootElement)[0];form.onsubmit=function(){return false;}
...
setTimeout(function(){jT.$(ketcherBox).css('display','none');},500);setTimeout(function(){jT.$(ketcherBox).toggleClass('shrinken')},100);});};cls.prototype={modifyUri:function(uri){var form=jT.$('form',this.rootElement)[0];var type=jT.$('input[name="searchtype"]:checked',form).val();var res=queries[type]+(uri.indexOf('?')>-1?'':'?')+uri;var params={type:this.search.type};if(!!this.search.mol)
params.b64search=$.base64.encode(this.search.mol);else{params.search=form.searchbox.value;if(!params.search)
