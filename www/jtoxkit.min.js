...
if(height==0)
break;for(i=0;i<tabs.length;++i){if(tabs[i]!=null){jQuery(tabs[i]).height(height);tabs[i]=tabs[i].nextElementSibling;}}}},positionTo:function(el,parent){var ps={left:0,top:0};for(;!!el&&el!=parent;el=el.offsetParent){ps.left+=el.offsetLeft;ps.top+=el.offsetTop;}
...
cell.removeAttribute('colspan');};var fnShowDetails=(self.settings.hasDetails?function(row,event){var cell=jT.$(".jtox-ds-details",row)[0];var idx=jT.$(row).data('jtox-index');jT.$(row).toggleClass('jtox-detailed-row');var toShow=jT.$(row).hasClass('jtox-detailed-row');fnExpandCell(cell,toShow);var varCell=document.getElementById('jtox-var-'+self.instanceNo+'-'+idx).firstElementChild;fnExpandCell(varCell,toShow);var iconCell=jT.$('.jtox-details-open',row);jT.$(iconCell).toggleClass('ui-icon-circle-triangle-e');jT.$(iconCell).toggleClass('ui-icon-circle-triangle-w');var detDiv=jT.$('.jtox-details-box',self.rootElement)[0];if(toShow){var full=self.dataset.dataEntry[idx];var width=jT.$(cell).width()+jT.$('.jtox-ds-variable',self.rootElement).width();jT.$(detDiv).width(width);var ps=ccLib.positionTo(cell,detDiv.parentNode);detDiv.style.left=ps.left+'px';detDiv.style.top=ps.top+'px';if(!ccLib.isNull(self.settings.detailsHeight)){if(parseInt(self.settings.detailsHeight)>0)
jT.$(detDiv).height(self.settings.detailsHeight);else if(self.settings.detailsHeight=='fill')
jT.$(detDiv).height(jT.$(cell).height()*2);}
jT.$(detDiv).toggleClass('jtox-hidden');self.prepareTabs(detDiv,false,function(id,name,parent){var fEl=null;if(id!=null){fEl=jT.getTemplate('#jtox-one-detail');parent.appendChild(fEl);ccLib.fillTree(fEl,{title:name,value:self.featureValue(id,full,'details'),uri:self.featureUri(id)});}
return fEl;},function(id,parent){var tabTable=jT.getTemplate('#jtox-details-table');parent.appendChild(tabTable);return tabTable;});jT.$(cell).height(detDiv.offsetHeight);ccLib.fireCallback(self.settings.onDetails,self,detDiv,full,event);}
else{jT$(detDiv).toggleClass('jtox-hidden');jT.$(cell).empty();jT.$(varCell).empty();}
