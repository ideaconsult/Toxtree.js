...
return false;})});$(form).on('submit',function(e){var empty=false;$('.non-empty',this).each(function(){if(self.isNull(this.value)||this.value.length==0){self.fireCallback(callback,this);empty=true;}});if(empty){e.preventDefault();return false;}});},clearChildren:function(obj,last){while(obj.lastChild&&obj.lastChild!=last){obj.removeChild(obj.lastChild);}},formatString:function(format){for(var i=1;i<arguments.length;++i){format=format.replace('<'+i+'>',arguments[i]);}
