...
return dataset;};return cls;})();var jToxDataset=(function(){var defaultSettings={shortStars:false,selectable:false,sDom:"<Fi>rt",configuration:{columns:{dataset:{'Id':{iOrder:0,sTitle:"Id",mData:"URI",sWidth:"50px",mRender:function(data,type,full){var num=parseInt(data.match(/http:\/\/.*\/dataset\/([0-9]+).*/)[1]);if(type!='display')
return num;return'<a target="_blank" href="'+data+'"><span class="ui-icon ui-icon-link jtox-inline"></span> D'+num+'</a>';}},'Title':{iOrder:1,sTitle:"Title",mData:"title",sDefaultContent:"-"},'Stars':{iOrder:2,sTitle:"Stars",mData:"stars",sWidth:"160px",mRender:function(data,type,full){if(type!='display')
return data;var res='<div title="Dataset quality stars rating (worst) 1-10 (best)">';for(var i=0;i<10;++i){res+='<span class="ui-icon ui-icon-star jtox-inline';if(i>data)
res+=' transparent';res+='"></span>';}
return res+'</div>';}},'Info':{iOrder:3,sTitle:"Info",mData:"rights",mRender:function(data,type,full){return'<a target="_blank" href="'+data.URI+'">rights</a>';}}}}}};var cls=function(root,settings){var self=this;self.rootElement=root;jT.$(root).addClass('jtox-toolkit');self.settings=jT.$.extend(true,{},defaultSettings,jT.settings,settings);self.rootElement.appendChild(jT.getTemplate('#jtox-dataset'));self.init();self.listDatasets(self.settings.listUri)};cls.prototype={init:function(){var self=this;if(self.settings.shortStars){self.settings.configuration.columns.dataset.Stars.mRender=function(data,type,full){if(type!='display')
return data;return'<span class="ui-icon ui-icon-star jtox-inline" title="Dataset quality stars rating (worst) 1-10 (best)"></span>'+data;};self.settings.configuration.columns.dataset.Stars.sWidth="40px";}
if(self.settings.selectable){var oldFn=self.settings.configuration.columns.dataset.Id.mRender;self.settings.configuration.columns.dataset.Id.mRender=function(data,type,full){var oldRes=oldFn(data,type,full);if(type!='display')
...
catCol={};return catCol;};col=this.$.extend(col,(!!group?getColDef('_'):{}),getColDef(category));return ccLib.isNull(col.bVisible)||col.bVisible?col:null;},sortColDefs:function(colDefs){colDefs.sort(function(a,b){var valA=ccLib.isNull(a.iOrder)?0:a.iOrder;var valB=ccLib.isNull(b.iOrder)?0:b.iOrder;return valA-valB;});},processColumns:function(kit,category){var colDefs=[];var catList=kit.settings.configuration.columns[category];for(var name in catList)
