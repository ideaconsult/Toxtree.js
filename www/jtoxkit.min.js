...
tabs[i].style.height=height+"px";tabs[i]=tabs[i].nextElementSibling;}}}},addParameter:function(url,param){return url+(url.indexOf('?')>0?"&":"?")+param;},makeURL:function(path){var a=document.createElement('a');a.href=path;return a.protocol+"//"+a.hostname+(a.port.length>0?":":'')+a.port+a.pathname.replace(/^([^\/])/,'/$1');},parseURL:function(url){var a=document.createElement('a');a.href=url;return{source:url,protocol:a.protocol.replace(':',''),host:a.hostname,port:a.port,query:a.search,params:(function(){var ret={},seg=a.search.replace(/^\?/,'').split('&'),len=seg.length,i=0,s;for(;i<len;i++){if(!seg[i]){continue;}
...
pane.style.display="none";};cls.prototype={prepareTabs:function(root,isMain,nodeFn,divFn){var self=this;var fr=document.createDocumentFragment();var all=document.createElement('div');fr.appendChild(all);ulEl=document.createElement('ul');all.appendChild(ulEl);var createATab=function(grId,name){var liEl=document.createElement('li');ulEl.appendChild(liEl);var aEl=document.createElement('a');aEl.href="#"+grId;aEl.innerHTML=name;liEl.appendChild(aEl);return liEl;};var emptyList=[];var idx=0;for(var gr in self.groups){var grId="jtox-ds-"+gr+"-"+self.instanceNo;createATab(grId,gr.replace(/_/g," "));var divEl=document.createElement('div');divEl.id=grId;all.appendChild(divEl);if(typeof divFn=='function'){divEl=divFn(gr,divEl);}
for(var i=0,glen=self.groups[gr].length;i<glen;++i){var fId=self.groups[gr][i];if(!ccLib.isNull(self.features[fId].title)){var el=nodeFn(fId,self.features[fId].title);if(!ccLib.isNull(el))
...
divEl.id=tabId;all.appendChild(divEl);divEl=divEl.getElementsByClassName('jtox-exportlist')[0];var base=jToxKit.grabBaseUrl(self.datasetUri,"dataset");for(var i=0,elen=self.settings.configuration.exports.length;i<elen;++i){var expo=self.settings.configuration.exports[i];var el=jToxKit.getTemplate('#jtox-ds-download');divEl.appendChild(el);el.getElementsByTagName('a')[0].href=ccLib.addParameter(self.datasetUri,"media="+expo.type);var img=el.getElementsByTagName('img')[0];img.alt=img.title=expo.type;img.src=base+expo.icon;}}
root.appendChild(fr);return $(all).tabs({collapsible:isMain,disabled:emptyList});},equalizeTables:function(){var self=this;if(!self.suspendEqualization&&self.fixTable!=null&&self.varTable!=null){ccLib.equalizeHeights(self.fixTable.tHead,self.varTable.tHead);ccLib.equalizeHeights(self.fixTable.tBodies[0],self.varTable.tBodies[0]);}},featureValue:function(fId,data){var self=this;var feature=self.features[fId];if(feature.accumulate!==undefined)
...
cell.removeAttribute('colspan');};var fnShowDetails=function(row){var cell=$(".jtox-ds-details",row)[0];var idx=$(row).data('jtox-index');$(row).toggleClass('jtox-detailed-row');var toShow=$(row).hasClass('jtox-detailed-row');fnExpandCell(cell,toShow);var varCell=document.getElementById('jtox-var-'+self.instanceNo+'-'+idx).firstElementChild;fnExpandCell(varCell,toShow);var iconCell=$('.jtox-details-open',row);$(iconCell).toggleClass('ui-icon-circle-triangle-s');$(iconCell).toggleClass('ui-icon-circle-triangle-n');if(toShow){var full=self.dataset.dataEntry[idx];var detDiv=document.createElement('div');varCell.appendChild(detDiv);var tabList=self.prepareTabs(detDiv,false,function(id,name){if(cls.shortFeatureId(id)=="Diagram")
return null;var fEl=jToxKit.getTemplate('#jtox-one-detail');ccLib.fillTree(fEl,{title:name,value:self.featureValue(id,full)});return fEl;},function(id,parent){var tabTable=jToxKit.getTemplate('#jtox-details-table');parent.appendChild(tabTable);return tabTable;});var img=new Image();img.onload=function(e){self.equalizeTables();$(detDiv).height(varCell.parentNode.clientHeight)
$(tabList).tabs("option","heightStyle","fill");};img.src=full.compound.diagramUri;cell.appendChild(img);}
...
else if(!!feature.shorten){col["mRender"]=function(data,type,full){return(type!="display")?''+data:jToxKit.shortenedData(data,"Press to copy the value in the clipboard");};col["sWidth"]="75px";}
$(checkList[checkIdx++]).on('change',fnShowColumn(colList==fixCols?'.jtox-ds-fixed':'.jtox-ds-variable',colList.length))
colList.push(col);}
colList=varCols;}
self.fixTable=($(".jtox-ds-fixed table",self.rootElement).dataTable({"bPaginate":false,"bProcessing":true,"bLengthChange":false,"bAutoWidth":true,"sDom":"rt","aoColumns":fixCols,"bSort":false,"fnCreatedRow":function(nRow,aData,iDataIndex){$('.jtox-details-open',nRow).on('click',function(e){fnShowDetails(nRow);});$(nRow).data('jtox-index',iDataIndex);},}))[0];self.varTable=($(".jtox-ds-variable table",self.rootElement).dataTable({"bPaginate":false,"bLengthChange":false,"bAutoWidth":false,"sDom":"rt","bSort":true,"aoColumns":varCols,"bScrollCollapse":true,"fnCreatedRow":function(nRow,aData,iDataIndex){nRow.id='jtox-var-'+self.instanceNo+'-'+iDataIndex;$(nRow).data('jtox-index',iDataIndex);$(nRow).addClass('jtox-row');},"fnDrawCallback":function(oSettings){var sorted=$('.jtox-row',this);var rlen=sorted.length;if(rlen==0)
return;var dataFeed=[];for(var i=0;i<rlen;++i)
dataFeed.push(self.dataset.dataEntry[$(sorted[i]).data('jtox-index')]);$(self.fixTable).dataTable().fnClearTable();$(self.fixTable).dataTable().fnAddData(dataFeed);self.suspendEqualization=true;if(self.settings.showTabs){$('.jtox-ds-features .jtox-checkbox',self.rootElement).trigger('change');}
self.suspendEqualization=false;self.equalizeTables();}}))[0];},updateTables:function(){var self=this;var fnFilterDataset=function(needle){if(ccLib.isNull(self.originalDataset))
self.originalDataset=self.dataset.dataEntry;var dataFeed=[];for(var i=0,slen=self.originalDataset.length;i<slen;++i){var entry=self.originalDataset[i];var match=self.enumerateFeatures(function(fId,gr){var feat=self.features[fId];if(feat.search!==undefined&&!feat.search)
return false;var val=self.featureValue(fId,entry);return!ccLib.isNull(val)&&val.toString().indexOf(needle)>=0;});if(match)
dataFeed.push(entry);}
console.log('Filtered to: '+dataFeed.length+' entries');self.dataset.dataEntry=dataFeed;};var needle=$('.jtox-ds-control input',self.rootElement).val();if(!ccLib.isEmpty(needle))
fnFilterDataset(needle);else if(!ccLib.isNull(self.originalDataset))
self.dataset.dataEntry=self.originalDataset;$(self.varTable).dataTable().fnClearTable();$(self.varTable).dataTable().fnAddData(self.dataset.dataEntry);},prepareGroups:function(){var self=this;var grps=self.settings.configuration.groups;self.groups={};for(var i in grps){var grp=grps[i];self.groups[i]=(typeof grp=="function")?grp(i,self.features):grp;}},enumerateFeatures:function(fnMatch){var self=this;var stopped=false;for(var gr in self.groups){for(var i=0,glen=self.groups[gr].length;i<glen;++i){if(stopped=fnMatch(self.groups[gr][i],gr))
break;}
if(stopped)
break;}
return stopped;},clearDataset:function(){var self=this;},nextPage:function(){var self=this;if(self.entriesCount===null||self.settings.pageStart+self.settings.pageSize<self.entriesCount)
...
self.queryEntries(self.settings.pageStart-self.settings.pageSize);},queryEntries:function(from,fnComplete){var self=this;if(from<0)
from=0;var qStart=Math.floor(from/self.settings.pageSize);var qUri=ccLib.addParameter(self.datasetUri,"page="+qStart+"&pagesize="+self.settings.pageSize);jToxKit.call(self,qUri,function(dataset){if(!!dataset){var qSize=dataset.dataEntry.length;qStart=self.settings.pageStart=qStart*self.settings.pageSize;if(qSize<self.settings.pageSize)
...
return oldVal;return oldVal+", "+newVal;},self.settings.pageStart);if(typeof fnComplete=='function')
fnComplete();self.originalDataset=null;self.updateTables();if(self.settings.showControls){var pane=$('.jtox-ds-control',self.rootElement)[0];ccLib.fillTree(pane,{"pagestart":qStart+1,"pageend":qStart+qSize,});var nextBut=$('.next-field',pane);if(self.entriesCount===null||qStart+qSize<self.entriesCount)
...
$(prevBut).addClass('paginate_disabled_previous').removeClass('paginate_enabled_previous');}}});},queryDataset:function(datasetUri){var self=this;self.clearDataset();self.datasetUri=datasetUri;jToxKit.call(self,datasetUri+'/feature',function(feature){if(!!feature){self.features=feature.feature;cls.processFeatures(self.features);self.prepareGroups();if(self.settings.showTabs){self.prepareTabs($('.jtox-ds-features',self.rootElement)[0],true,function(id,name){var fEl=jToxKit.getTemplate('#jtox-ds-feature');ccLib.fillTree(fEl,{title:name});$(fEl).data('feature-id',id);return fEl;});}
self.queryEntries(self.settings.pageStart,function(){self.prepareTables();})}});},};cls.shortFeatureId=function(fId){return fId.substr(fId.indexOf('#')+1);};cls.processEntry=function(entry,features,fnValue){for(var fid in entry.values){var feature=features[fid];if(feature.accumulate!==undefined){var accArr=feature.accumulate;if(!$.isArray(accArr))
...
if(!startIdx)
startIdx=0;for(var i=0,dl=dataset.dataEntry.length;i<dl;++i){cls.processEntry(dataset.dataEntry[i],features,fnValue);dataset.dataEntry[i].index=i+1+startIdx;var uri=dataset.dataEntry[i].compound.URI;uri=uri.replace(/(.+)(\/conformer.*)/,"$1");dataset.dataEntry[i].compound.diagramUri=uri+"?media=image/png";}
...
substance.showname=substance.name;ccLib.fillTree(self.rootElement,substance);jToxKit.call(self,substance.referenceSubstance.uri,function(dataset){if(!!dataset){jToxDataset.processDataset(dataset,null,fnDatasetValue);ccLib.fillTree(rootTab,dataset.dataEntry[0]);}});self.querySummary(substance.URI);self.queryComposition(substance.URI);}});}};return cls;})();window.jToxKit={templateRoot:null,queries:{taskPoll:"/task/<1>",},templates:{},settings:{jsonp:false,baseUrl:null,timeout:15000,pollDelay:200,onConnect:function(s){},onSuccess:function(c,m){},onError:function(c,m){console.log("jToxKit call error ("+c+"): "+m);},},init:function(){var self=this;self.initTemplates();$(document).on('click','.jtox-toolkit span.ui-icon-copy',function(e){ccLib.copyToClipboard($(this).data('uuid'));return false;});var url=ccLib.parseURL(document.location);var queryParams=url.params;queryParams.host=url.host;self.settings=$.extend(self.settings,queryParams);if(!self.settings.baseUrl)
self.settings.baseUrl=self.settings.host;var initKit=function(element,params){if(params.kit=="study")
new jToxStudy(element,params);if(params.kit=="dataset")
new jToxDataset(element,params);};$('.jtox-toolkit').each(function(i){var dataParams=$(this).data();if(!dataParams.manualInit){var el=this;if(!ccLib.isNull(dataParams.configFile)){self.call(null,ccLib.makeURL(dataParams.configFile),function(config){if(!!config)
dataParams['configuration']=config;initKit(el,dataParams);});}
else
initKit(el,dataParams);}});},initTemplates:function(){var self=this;var root=document.getElementsByClassName('jtox-template')[0];if(root===undefined){var html='';for(var t in self.templates){html+=self.templates[t];}
...
return this.settings.baseUrl;},call:function(kit,service,callback,adata){var settings={};if(kit==null){kit=this;settings=this.settings;}
else
settings=$.extend(true,settings,this.settings,kit.settings);ccLib.fireCallback(settings.onConnect,kit,service);var method='GET';var accType=settings.jsonp?"application/x-javascript":"application/json";if(adata!==undefined){method='POST';if(typeof adata=="boolean")
adata={};}
else
adata={};if(service.indexOf("http")!=0)
service=settings.baseUrl+service;$.ajax(service,{dataType:settings.jsonp?'jsonp':'json',headers:{Accept:accType},crossDomain:true,timeout:settings.timeout,type:method,data:adata,jsonp:settings.jsonp?'callback':false,error:function(jhr,status,error){ccLib.fireCallback(settings.onError,kit,status,error);callback(null);},success:function(data,status,jhr){ccLib.fireCallback(settings.onSuccess,kit,status,jhr.statusText);callback(data);}});}};$(document).ready(function(){jToxKit.init();});jToxKit.templates['all-dataset']="	  <div id=\"jtox-dataset\">"+
...
"    </div>"+
"";jToxKit.templates['dataset-one-detail']="    <tr id=\"jtox-one-detail\">"+
