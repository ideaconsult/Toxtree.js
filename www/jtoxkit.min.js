...
tabRoot.appendChild(detDiv);self.prepareTabs(detDiv,false,function(id,name,parent){var fEl=null;if(id!=null){fEl=jT.getTemplate('#jtox-one-detail');parent.appendChild(fEl);ccLib.fillTree(fEl,{title:name,value:self.featureValue(id,full,'details'),uri:self.featureUri(id)});}
return fEl;},function(id,parent){var tabTable=document.createElement('table');tabTable.className='jtox-details-table';parent.appendChild(tabTable);return tabTable;});jT.$(cell).height(detDiv.offsetHeight);ccLib.fireCallback(self.settings.onDetails,self,detDiv,full,event);jT.$(cell).data('detailsDiv',detDiv);jT.$(detDiv).data('rootCell',cell);}
...
continue;var grpArr=(typeof grp=="function"||typeof grp=="string")?ccLib.fireCallback(grp,self,i,miniset):grp;self.groups[i]=[];ccLib.enumObject(grpArr,function(fid,idx){var sameAs=cls.findSameAs(fid,self.feature);if(!self.feature[sameAs].used&&!self.feature[fid].used)
self.groups[i].push(fid);if(idx!="name"){self.usedFeatures.push(fid);self.usedFeatures.push(sameAs);self.feature[fid].used=self.feature[sameAs].used=true;}});}},enumerateFeatures:function(fnMatch){var self=this;var stopped=false;for(var gr in self.groups){for(var i=0,glen=self.groups[gr].length;i<glen;++i){if(stopped=fnMatch(self.groups[gr][i],gr))
break;}
if(stopped)
break;}
