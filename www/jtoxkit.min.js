...
if(isMain&&self.settings.showExport){var tabId="jtox-ds-export-"+instanceCount;var liEl=createATab(tabId,"Export");$(liEl).addClass('jtox-ds-export');var divEl=jToxKit.getTemplate('#jtox-ds-export')
...
info[aoData[i].name]=aoData[i].value;var page=info.iDisplayStart/info.iDisplayLength;var qUri=ccLib.addParameter(self.datasetUri,"page="+page+"&pagesize="+info.iDisplayLength);jToxKit.call(self,qUri,function(dataset){if(!!dataset){cls.processDataset(dataset,self.features,function(oldVal,newVal){if(ccLib.isNull(oldVal)||newVal.toLowerCase().indexOf(oldVal.toLowerCase())>=0)
...
return count;}
var formatLoHigh=function(data,type){var out="";data.loValue=ccLib.trim(data.loValue);data.upValue=ccLib.trim(data.upValue);if(!ccLib.isEmpty(data.loValue)&&!ccLib.isEmpty(data.upValue)){out+=(data.loQualifier==">=")?"[":"(";out+=data.loValue+", "+data.upValue;out+=(data.upQualifier=="<=")?"]":") ";}
else
...
"";jToxKit.templates['dataset-export']="    <div id=\"jtox-ds-download\" class=\"jtox-inline\">"+
...
"    </div>"+
"";jToxKit.templates['dataset-export']="    <div id=\"jtox-ds-details\">"+
