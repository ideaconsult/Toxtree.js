...
'/>'+oldRes;}};return cls;})();var jToxModel=(function(){var defaultSettings={selectable:false,selectionHandler:null,maxStars:10,algorithmLink:true,algorithms:false,algorithmNeedle:null,sDom:"<Fif>rt",configuration:{columns:{model:{'Id':{iOrder:0,sTitle:"Id",mData:"id",sWidth:"50px",mRender:function(data,type,full){if(type!='display')
...
uri=self.settings.baseUrl+'/model';self.modelUri=uri;jT.$(self.table).dataTable().fnClearTable();jT.call(self,uri,function(result){if(!!result){self.models=result.model;jT.$(self.table).dataTable().fnAddData(result.model);}});},listAlgorithms:function(needle){var self=this;var uri=self.settings.baseUrl+'/algorithm';if(!!needle)
uri=ccLib.addParameter(uri,'search='+needle);jT.call(self,uri,function(result){if(!!result){self.algorithms=result.algorithm;jT.$(self.table).dataTable().fnAddData(result.algorithm);}});},getModel:function(algoUri,callback){},runPrediction:function(compoundUri,modelUri,callback){},query:function(uri){if(this.settings.algorithms)
