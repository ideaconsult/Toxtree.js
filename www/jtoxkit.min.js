...
for(;;){var height=0;for(i=0;i<tabs.length;++i){if(tabs[i]!=null&&tabs[i].offsetHeight>height)
height=tabs[i].offsetHeight;}
if(height==0)
break;for(i=0;i<tabs.length;++i){if(tabs[i]!=null&&tabs[i].offsetHeight<height)
tabs[i].style.height=height+"px";tabs[i]=tabs[i].nextElementSibling;}}},parseURL:function(url){var a=document.createElement('a');a.href=url;return{source:url,protocol:a.protocol.replace(':',''),host:a.hostname,port:a.port,query:a.search,params:(function(){var ret={},seg=a.search.replace(/^\?/,'').split('&'),len=seg.length,i=0,s;for(;i<len;i++){if(!seg[i]){continue;}
...
var jToxDataset=(function(){var defaultSettings={"groups":{"Identifiers":function(name,features){return["http://www.opentox.org/api/1.1#Diagram","http://www.opentox.org/api/1.1#CASRN","http://www.opentox.org/api/1.1#EINECS","http://www.opentox.org/api/1.1#ChemicalName","http://www.opentox.org/api/1.1#SMILES","http://www.opentox.org/api/1.1#InChI","http://www.opentox.org/api/1.1#REACHRegistrationDate"];},"Names":function(name,features){return["http://www.opentox.org/api/1.1#ChemicalName","http://www.opentox.org/api/1.1#TradeName","http://www.opentox.org/api/1.1#IUPACName"];}}};var instanceCount=0;var baseFeatures={"http://www.opentox.org/api/1.1#REACHRegistrationDate":{title:"REACH Date",accumulate:"compound.reachdate"},"http://www.opentox.org/api/1.1#CASRN":{title:"CAS",accumulate:"compound.cas"},"http://www.opentox.org/api/1.1#ChemicalName":{title:"Name",accumulate:"compound.name"},"http://www.opentox.org/api/1.1#TradeName":{title:"Trade Name",accumulate:"compound.tradename"},"http://www.opentox.org/api/1.1#IUPACName":{title:"IUPAC Name",accumulate:["compound.name","compound.iupac"]},"http://www.opentox.org/api/1.1#EINECS":{title:"EINECS",accumulate:"compound.einecs"},"http://www.opentox.org/api/1.1#InChI":{title:"InChI",accumulate:"compound.inchi"},"http://www.opentox.org/api/1.1#InChI_std":{title:"InChI",accumulate:"compound.inchi"},"http://www.opentox.org/api/1.1#InChIKey":{title:"InChI Key",accumulate:"compound.inchikey"},"http://www.opentox.org/api/1.1#InChIKey_std":{title:"InChI Key",accumulate:"compound.inchikey"},"http://www.opentox.org/api/1.1#InChI_AuxInfo":{title:"InChI Aux",accumulate:"compound.inchi"},"http://www.opentox.org/api/1.1#InChI_AuxInfo_std":{title:"InChI Aux",accumulate:"compound.inchi"},"http://www.opentox.org/api/1.1#IUCLID5_UUID":{title:"IUCLID5 UUID",accumulate:"compound.i5uuid"},"http://www.opentox.org/api/1.1#SMILES":{title:"SMILES",accumulate:"compound.smiles"},"http://www.opentox.org/api/dblinks#CMS":{title:"CMS"},"http://www.opentox.org/api/dblinks#ChEBI":{title:"ChEBI"},"http://www.opentox.org/api/dblinks#Pubchem":{title:"Public Chem"},"http://www.opentox.org/api/dblinks#ChemSpider":{title:"Chem Spider"},"http://www.opentox.org/api/dblinks#ChEMBL":{title:"ChEMBL"},"http://www.opentox.org/api/dblinks#ToxbankWiki":{title:"Toxban Wiki"},"http://www.opentox.org/api/1.1#Diagram":{title:"Diagram",accumulate:"compound.URI"}};var commonFeatures={};var cls=function(root,settings){var self=this;self.rootElement=root;self.settings=$.extend({},defaultSettings,jToxKit.settings,settings);self.features=null;self.groups=null;root.appendChild(jToxKit.getTemplate('#jtox-dataset'));instanceCount++;};cls.prototype={prepareTabs:function(root,nodeFn){var self=this;var fr=document.createDocumentFragment();var all=document.createElement('div');fr.appendChild(all);ulEl=document.createElement('ul');all.appendChild(ulEl);var featNames={};for(var gr in self.groups){var grId="jtox-ds-"+gr+"-"+instanceCount;var liEl=document.createElement('li');ulEl.appendChild(liEl);var aEl=document.createElement('a');aEl.href="#"+grId;aEl.innerHTML=gr.replace(/_/g," ");liEl.appendChild(aEl);var divEl=document.createElement('div');divEl.id=grId;for(var i=0,glen=self.groups[gr].length;i<glen;++i){var fId=self.groups[gr][i];divEl.appendChild(nodeFn(fId,self.features[fId].title));}
all.appendChild(divEl);}
root.appendChild(fr);return $(all).tabs();},prepareTables:function(){var self=this;var fixedCols=[];var varCols=[];var colList=fixedCols;fixedCols.push({"mData":"index"});for(var gr in self.groups){for(var i=0,glen=self.groups[gr].length;i<glen;++i){var fId=self.groups[gr][i];var feature=self.features[fId];var col={"sTitle":feature.title+(ccLib.isNull(feature.units)?"":feature.units),"sDefaultContent":"-","mData":feature.accumulate!==undefined?feature.accumulate:"values."+fId};if(cls.shortFeatureId(fId)=="Diagram"){col["mRender"]=function(data,type,full){return(type!="display")?"-":'<img src="'+full.compound.diagramUri+'" class="jtox-ds-smalldiagram"/>';}
col["sClass"]="paddingless";col["sWidth"]="125px";}
colList.push(col);}
colList=varCols;}
var varTable=($(".jtox-ds-variable table",self.rootElement).dataTable({"bPaginate":false,"bProcessing":false,"bLengthChange":false,"bAutoWidth":true,"sDom":"rt","aoColumns":varCols}))[0];var fixTable=($(".jtox-ds-fixed table",self.rootElement).dataTable({"bPaginate":true,"bProcessing":true,"bLengthChange":false,"bAutoWidth":false,"sDom":"rt<Fip>","aoColumns":fixedCols,"bServerSide":true,"fnServerData":function(sSource,aoData,fnCallback,oSettings){var info={};for(var i=0,dl=aoData.length;i<dl;++i)
info[aoData[i].name]=aoData[i].value;var page=info.iDisplayStart/info.iDisplayLength;var qUri=self.datasetUri+(self.datasetUri.indexOf('?')>0?'&':'?')+"page="+page+"&pagesize="+info.iDisplayLength;jToxKit.call(self,qUri,function(dataset){if(!!dataset){cls.processDataset(dataset,null,self.features);$(varTable).dataTable().fnClearTable();$(varTable).dataTable().fnAddData(dataset.dataEntry);fnCallback({"sEcho":info.sEcho,"iTotalRecords":dataset.query.total,"iTotalDisplayRecords":dataset.dataEntry.length,"aaData":dataset.dataEntry});ccLib.equalizeHeights(fixTable.tBodies[0],varTable.tBodies[0]);}});}}))[0];ccLib.equalizeHeights(fixTable.tHead,varTable.tHead);},prepareGroups:function(){var self=this;var grps=self.settings.groups;self.groups={};for(var i in grps){self.groups[i]=(grps[i])(i,self.features);}},clearDataset:function(){var self=this;},queryDataset:function(datasetUri){var self=this;self.clearDataset();self.datasetUri=datasetUri;jToxKit.call(self,datasetUri+'/feature',function(feature){if(!!feature){self.features=feature.feature;cls.processFeatures(self.features);self.prepareGroups();self.prepareTabs($('.jtox-ds-features',self.rootElement)[0],function(id,name){var fEl=jToxKit.getTemplate('#jtox-ds-feature');ccLib.fillTree(fEl,{title:name});$(fEl).data('feature-id',id);return fEl;});self.prepareTables();}});},};cls.shortFeatureId=function(fId){return fId.substr(fId.indexOf('#')+1);};cls.processEntry=function(entry,features,fnValue){for(var fid in entry.values){var feature=features[fid];if(feature.accumulate!==undefined){var accArr=feature.accumulate;if(!$.isArray(accArr))
