...
return dataset;};return cls;})();var jToxDataset=(function(){var defaultSettings={shortStars:false,maxStars:10,selectable:false,selectionHandler:null,sDom:"<Fif>rt",onLoaded:null,loadOnInit:false,configuration:{columns:{dataset:{'Id':{iOrder:0,sTitle:"Id",mData:"URI",sWidth:"50px",mRender:function(data,type,full){var num=parseInt(data.match(/http:\/\/.*\/dataset\/(\d+).*/)[1]);if(type!='display')
...
self.settings.configuration.columns.dataset.Stars.sWidth="40px";if(self.settings.selectable){jT.ui.putActions(self,self.settings.configuration.columns.dataset.Id,{selection:true});self.settings.configuration.columns.dataset.Id.sWidth="60px";}
...
return res;};}
var cat=self.settings.algorithms?'algorithm':'model';if(self.settings.selectable){jT.ui.putActions(self,self.settings.configuration.columns[cat].Id,{selection:true});self.settings.configuration.columns[cat].Id.sWidth="60px";}
...
self.querySubstance(self.settings.substanceUri)};cls.prototype={init:function(){var self=this;var colId=self.settings.configuration.columns.substance.Id;jT.ui.putActions(self,colId,{selection:self.settings.selectable,details:self.settings.hasDetails});colId.sName='';var fnShowDetails=!self.settings.hasDetails?null:function(row,e){jT.$(e.currentTarget).toggleClass('ui-icon-circle-triangle-e');jT.$(e.currentTarget).toggleClass('ui-icon-circle-triangle-w');alert('Show/hide details');};jT.ui.putControls(self,{nextPage:function(){self.nextPage();},prevPage:function(){self.prevPage();},sizeChange:function(){self.queryEntries(self.settings.pageStart,parseInt(jT.$(this).val()));},filter:function(){jT.$(self.table).dataTable().fnFilter(jT.$(this).val());}});self.table=jT.$('table',self.rootElement).dataTable({"bPaginate":false,"bProcessing":true,"bLengthChange":false,"bAutoWidth":false,"sDom":"rt","aoColumns":jT.ui.processColumns(self,'substance'),"fnCreatedRow":function(nRow,aData,iDataIndex){if(self.settings.hasDetails)
jT.$('.jtox-details-open',nRow).on('click',function(e){fnShowDetails(nRow,e);});},"bServerSide":false,"oLanguage":{"sLoadingRecords":"No substances found.","sZeroRecords":"No substances found.","sEmptyTable":"No substances available.","sInfo":"Showing _TOTAL_ substance(s) (_START_ to _END_)"}});jT.$(self.table).dataTable().fnAdjustColumnSizing();},queryEntries:function(from,size){var self=this;if(from<0)
...
return res+'</div>';}
else{return'<span class="ui-icon ui-icon-star jtox-inline" title="'+title+'"></span>'+stars;}},putActions:function(kit,col,defs){if(!!defs&&!jQuery.isEmptyObject(defs)){var oldFn=col.mRender;var newFn=function(data,type,full){var html=oldFn(data,type,full);if(type!='display')
return html;if(!!defs.ignoreOriginal)
html='';if(typeof defs.selection=='function')
...
pane.style.display="none";}};jT.$=jQuery;jT.$(document).ready(function(){jT.init();});jT.templates['widget-search']="    <div id=\"jtox-search\" class=\"jtox-search\">"+
"      <form>"+
