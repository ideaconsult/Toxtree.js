...
root.appendChild(fr);return $(all).tabs({collapsible:isMain});},prepareTables:function(){var self=this;var fixedCols=[];var varCols=[];var colList=fixedCols;fixedCols.push({"mData":"index","sClass":"middle"});var fnShowColumn=function(sel,idx){return function(){$(sel+' table',self.rootElement).dataTable().fnSetColumnVis(idx,this.checked);ccLib.equalizeHeights(self.fixTable.tHead,self.varTable.tHead);ccLib.equalizeHeights(self.fixTable.tBodies[0],self.varTable.tBodies[0]);}};var checkList=$('.jtox-ds-features .jtox-checkbox',self.rootElement);var checkIdx=0;for(var gr in self.groups){for(var i=0,glen=self.groups[gr].length;i<glen;++i){var fId=self.groups[gr][i];var feature=self.features[fId];var col={"sTitle":feature.title+(ccLib.isNull(feature.units)?"":feature.units),"sDefaultContent":"-","mData":feature.accumulate!==undefined?feature.accumulate:"values."+fId};if(cls.shortFeatureId(fId)=="Diagram"){col["mRender"]=function(data,type,full){return(type!="display")?"-":'<img src="'+full.compound.diagramUri+'" class="jtox-ds-smalldiagram"/>';}
...
colList.push(col);}
colList=varCols;}
self.varTable=($(".jtox-ds-variable table",self.rootElement).dataTable({"bPaginate":false,"bProcessing":false,"bLengthChange":false,"bAutoWidth":true,"sDom":"rt","bSort":false,"aoColumns":varCols}))[0];self.fixTable=($(".jtox-ds-fixed table",self.rootElement).dataTable({"bPaginate":true,"bProcessing":true,"bLengthChange":false,"bAutoWidth":false,"sDom":"rt<Fip>","aoColumns":fixedCols,"bServerSide":true,"bSort":false,"fnServerData":function(sSource,aoData,fnCallback,oSettings){var info={};for(var i=0,dl=aoData.length;i<dl;++i)
