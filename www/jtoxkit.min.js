...
box.value=url;}};return cls;})();var jToxCompound=(function(){var defaultSettings={"showTabs":true,"showExport":true,"showControls":true,"pageSize":20,"pageStart":0,"metricFeature":"http://www.opentox.org/api/1.1#Similarity","fnAccumulate":function(fId,oldVal,newVal,features){if(ccLib.isNull(newVal))
...
return dataset;};return cls;})();var jToxDataset=(function(){var defaultSettings={shortStars:false,maxStars:10,selectable:false,sDom:"<Fif>rt",configuration:{columns:{dataset:{'Id':{iOrder:0,sTitle:"Id",mData:"URI",sWidth:"50px",mRender:function(data,type,full){var num=parseInt(data.match(/http:\/\/.*\/dataset\/(\d+).*/)[1]);if(type!='display')
...
uri=self.settings.baseUrl+'/dataset';jT.$(self.table).dataTable().fnClearTable();jT.call(self,uri,function(result){if(!!result){jT.$(self.table).dataTable().fnAddData(result.dataset);}});},query:function(uri){this.listDatasets(uri);},modifyUri:function(uri){jT.$('input[type="checkbox"]',this.rootElement).each(function(){if(this.checked)
...
return oldRes;return'<input type="checkbox" value="'+full.URI+'"'+
(!!kit.settings.selectionHandler?' class="jtox-handler" data-handler="'+kit.settings.selectionHandler+'"':'')+
'/>'+oldRes;}};return cls;})();var jToxModel=(function(){var defaultSettings={selectable:false,maxStars:10,algorithmFilter:true,sDom:"<Fif>rt",configuration:{columns:{model:{'Id':{iOrder:0,sTitle:"Id",mData:"id",sWidth:"50px",mRender:function(data,type,full){if(type!='display')
...
return data;return'<a href="'+data+'"><span class="ui-icon ui-icon-calculator"></span>&nbsp;training set</a>';}}}}}};var cls=function(root,settings){var self=this;self.rootElement=root;jT.$(root).addClass('jtox-toolkit');self.settings=jT.$.extend(true,{},defaultSettings,jT.settings,settings);self.models=null;self.rootElement.appendChild(jT.getTemplate('#jtox-model'));self.init();self.listModels(self.settings.listUri)};cls.prototype={init:function(){var self=this;self.settings.configuration.columns.model.Stars.mRender=function(data,type,full){return type!='display'?data:jToxDataset.putStars(self,data,"Model star rating (worst) 1 - 10 (best)");};if(self.settings.shortStars){self.settings.configuration.columns.model.Stars.sWidth="40px";}
if(self.settings.selectable){self.settings.configuration.columns.model.Id.mRender=jToxDataset.addSelection(self,self.settings.configuration.columns.model.Id.mRender);self.settings.configuration.columns.model.Id.sWidth="60px";}
...
name+
'</a>';if(self.settings.algorithmFilter){res+='<a href="'+ccLib.addParameter(self.modelUri,'algorithm='+encodeURIComponent(data.URI))+'"><span class="ui-icon ui-icon-calculator float-right" title="Show models using algorithm '+name+'"></span></a>';}
return res;};self.table=jT.$('table',self.rootElement).dataTable({"bPaginate":false,"bLengthChange":false,"bAutoWidth":false,"sDom":self.settings.sDom,"aoColumns":jT.processColumns(self,'model'),"oLanguage":{"sLoadingRecords":"No models found.","sZeroRecords":"No models found.","sEmptyTable":"No models available.","sInfo":"Showing _TOTAL_ model(s) (_START_ to _END_)"}});jT.$(self.table).dataTable().fnAdjustColumnSizing();},listModels:function(uri){var self=this;if(uri==null)
uri=self.settings.baseUrl+'/model';self.modelUri=uri;jT.$(self.table).dataTable().fnClearTable();jT.call(self,uri,function(result){if(!!result){self.models=result.model;jT.$(self.table).dataTable().fnAddData(result.model);}});},runPrediction:function(modelUri,compoundUri,callback){},query:function(uri){this.listModels(uri);},modifyUri:function(uri){jT.$('input[type="checkbox"]',this.rootElement).each(function(){if(this.checked)
...
dataParams.configuration=window[dataParams.configuration];jT.$(element).data('jtKit',realInit(dataParams));}}},init:function(root){var self=this;if(!root){self.initTemplates();jT.$(document).on('click','.jtox-toolkit span.ui-icon-copy',function(e){ccLib.copyToClipboard(jT.$(this).data('uuid'));return false;});var url=self.settings.fullUrl=ccLib.parseURL(document.location);var queryParams=url.params;if(!queryParams.baseUrl)
