...
self.querySubstance(self.settings.substanceUri)};cls.prototype={init:function(){var self=this;var colId=self.settings.configuration.columns.substance.Id;jT.ui.putActions(self,colId,{selection:self.settings.selectionHandler,details:!!self.settings.onDetails});colId.sName='';jT.ui.putControls(self,{nextPage:function(){self.nextPage();},prevPage:function(){self.prevPage();},sizeChange:function(){self.queryEntries(self.settings.pageStart,parseInt(jT.$(this).val()));},filter:function(){jT.$(self.table).dataTable().fnFilter(jT.$(this).val());}});self.table=jT.$('table',self.rootElement).dataTable({"bPaginate":false,"bProcessing":true,"bLengthChange":false,"bAutoWidth":false,"sDom":"rt","aoColumns":jT.ui.processColumns(self,'substance'),"fnCreatedRow":function(nRow,aData,iDataIndex){if(!!self.settings.onDetails)
