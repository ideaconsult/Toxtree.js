...
substance.showname=substance.name;ccLib.fillTree(self.rootElement,substance);jToxKit.call(self,substance.referenceSubstance.uri,function(dataset){if(!!dataset){jToxDataset.processDataset(dataset,null,fnDatasetValue);ccLib.fillTree(rootTab,dataset.dataEntry[0]);}});self.querySummary(substance.URI);self.queryComposition(substance.URI);}});}};return cls;})();window.jToxKit={templateRoot:null,queries:{taskPoll:"/task/<1>",},templates:{},settings:{jsonp:false,crossDomain:false,baseUrl:null,timeout:15000,pollDelay:200,onConnect:function(s){},onSuccess:function(s,c,m){},onError:function(s,c,m){if(!!console&&!!console.log)console.log("jToxKit call error ("+c+"): "+m+" from request: ["+s+"]");},},init:function(){var self=this;self.initTemplates();$(document).on('click','.jtox-toolkit span.ui-icon-copy',function(e){ccLib.copyToClipboard($(this).data('uuid'));return false;});var url=ccLib.parseURL(document.location);var queryParams=url.params;queryParams.host=url.host;self.settings=$.extend(self.settings,queryParams);if(!self.settings.baseUrl)
