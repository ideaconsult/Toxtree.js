...
processFn(root,json);jQuery('.'+filter,root).each(function(i){processFn(jQuery(this)[0],json);});if(prefix&&json.id!==undefined){root.id=prefix+json.id;}},prepareForm:function(form,callback){var self=this;var $=window.jQuery;$('.accumulate',form).each(function(){$(this).on('change',function(e){var target=$(this).data('accumulate');if(!!target){target=this.form[target];var val=target.value.replace(new RegExp('('+this.value+')'),'');if(this.checked)
val+=','+this.value;target.value=val.replace(/,,/g,',').replace(/^,/g,'').replace(/,$/g,'');}
return false;})});$(form).on('submit',function(e){var empty=null;$('.non-empty',this).each(function(){if(self.isNull(this.value)||this.value.length==0)
empty=this;});if(!!empty){self.fireCallback(callback,null,empty);e.preventDefault();return false;}});},clearChildren:function(obj,last){while(obj.lastChild&&obj.lastChild!=last){obj.removeChild(obj.lastChild);}},formatString:function(format){for(var i=1;i<arguments.length;++i){format=format.replace('<'+i+'>',arguments[i]);}
...
jT.$(theTable).dataTable().fnClearTable();return theTable;},processSummary:function(summary){var self=this;var typeSummary=[];jT.$('.jtox-study',self.rootElement).remove();summary.sort(function(a,b){var valA=(a.category.description||a.category.title);var valB=(b.category.description||b.category.title);if(valA==null)
...
self.processSummary(summary.facet);});},queryComposition:function(compositionURI){var self=this;jT.call(self,compositionURI,function(composition){if(!!composition&&!!composition.composition)
...
substance["IUCFlags"]=flags;ccLib.fillTree(self.rootElement,substance);jT.call(self,substance.referenceSubstance.uri,function(dataset){if(!!dataset){jToxDataset.processDataset(dataset,null,fnDatasetValue);ccLib.fillTree(rootTab,dataset.dataEntry[0]);}});self.querySummary(substance.URI+"/studysummary");self.queryComposition(substance.URI+"/composition");}});}};return cls;})();window.jT=window.jToxKit={templateRoot:null,queries:{taskPoll:"/task/<1>",},templates:{},tools:{},settings:{jsonp:false,crossDomain:false,host:null,timeout:15000,pollDelay:200,onConnect:function(s){},onSuccess:function(s,c,m){},onError:function(s,c,m){if(!!console&&!!console.log)console.log("jToxKit call error ("+c+"): "+m+" from request: ["+s+"]");},},formBaseUrl:function(url){return url.protocol+"://"+url.host+(url.port.length>0?":"+url.port:'')+'/'+url.segments[0]+'/';},init:function(){var self=this;self.initTemplates();jT.$(document).on('click','.jtox-toolkit span.ui-icon-copy',function(e){ccLib.copyToClipboard(jT.$(this).data('uuid'));return false;});var url=ccLib.parseURL(document.location);var queryParams=url.params;queryParams.host=self.formBaseUrl(url);self.settings=jT.$.extend(self.settings,queryParams);var initKit=function(element,params){var kit=params.kit;if(!kit)
return null;if(kit.indexOf('jTox')!=0)
kit='jTox'+kit.charAt(0).toUpperCase()+kit.slice(1);var fn=window[kit];return(typeof fn=='function')?new fn(element,params):null;};jT.$('.jtox-toolkit').each(function(i){var dataParams=jT.$.extend(true,self.settings,jT.$(this).data());if(!dataParams.manualInit){var el=this;if(!ccLib.isNull(dataParams.configFile)){self.call({settings:dataParams},dataParams.configFile,function(config){if(!!config)
...
root.innerHTML=html;return root;},changeTabsIds:function(root,suffix){jT.$('ul li a',root).each(function(){var id=jT.$(this).attr('href').substr(1);var el=document.getElementById(id);id+=suffix;el.id=id;jT.$(this).attr('href','#'+id);})},modifyColDef:function(kit,col,category,group){if(group==null)
group="main";var name=col.sTitle.toLowerCase();var getColDef=function(cat){var catCol=kit.settings.configuration.columns[cat];if(!ccLib.isNull(catCol)){catCol=catCol[group];if(!ccLib.isNull(catCol))
catCol=catCol[name];}
if(ccLib.isNull(catCol))
catCol={};return catCol;};col=this.$.extend(col,getColDef('_'),getColDef(category));return ccLib.isNull(col.bVisible)||col.bVisible?col:null;},sortColDefs:function(colDefs){colDefs.sort(function(a,b){var valA=ccLib.isNull(a.iOrder)?0:a.iOrder;var valB=ccLib.isNull(b.iOrder)?0:b.iOrder;return valA-valB;});},shortenedData:function(data,message,deflen){var res='';if(ccLib.isNull(deflen))
...
return this.settings.host;},call:function(kit,service,params,callback){if(typeof params!='object'){callback=params;params={};}
var settings=jT.$.extend({"baseUrl":this.settings.host},this.settings,params);if(kit==null)
kit=this;else
settings=jT.$.extend(settings,kit.settings);ccLib.fireCallback(settings.onConnect,kit,service);var accType=settings.jsonp?"application/x-javascript":"application/json";if(params.data===undefined){params.data={};if(settings.jsonp)
params.data.media=accType;if(params.method===undefined)
params.method='GET';}
else if(params.method===undefined)
params.method='POST';if(service.indexOf("http")!=0)
service=settings.baseUrl+service;jT.$.ajax(service,{dataType:settings.jsonp?'jsonp':'json',headers:{Accept:accType},crossDomain:settings.crossDomain||settings.jsonp,timeout:settings.timeout,type:params.method,data:params.data,jsonp:settings.jsonp?'callback':false,error:function(jhr,status,error){ccLib.fireCallback(settings.onError,kit,service,status,error);callback(null,jhr);},success:function(data,status,jhr){ccLib.fireCallback(settings.onSuccess,kit,service,status,jhr.statusText);callback(data,jhr);}});}};jT.$=jQuery;jT.$(document).ready(function(){jT.init();});jT.templates['all-dataset']="	  <div id=\"jtox-dataset\">"+
