...
obj.innerHTML=value;},isNull:function(obj){return obj===undefined||obj==null;},isEmpty:function(obj){var empty=true;if(obj!==undefined||obj!=null){for(var i in obj){if(obj[i]!=null){empty=false;break;}}}
...
return count;}
var formatLoHigh=function(data,type){var out="";data.loValue=ccLib.trim(data.loValue);data.upValue=ccLib.trim(data.upValue);if(!ccLib.isNull(data.loValue)&&!ccLib.isNull(data.upValue)){out+=(data.loQualifier==">=")?"[":"(";out+=data.loValue+", "+data.upValue;out+=(data.upQualifier=="<=")?"]":") ";}
else
{var fnFormat=function(q,v){return(!!q?q:"=")+" "+v;};if(!ccLib.isNull(data.loValue))
out+=fnFormat(data.loQualifier,data.loValue);else if(!ccLib.isNull(data.upValue))
out+=fnFormat(data.upQualifier,data.upValue);else
out+='-';}
data.unit=ccLib.trim(data.unit);if(!!data.unit)
out+=data.unit;return out.replace(/ /g,"&nbsp;");};var formatUnits=function(data,unit){data=ccLib.trim(data);unit=ccLib.trim(unit);return!ccLib.isNull(data)?(data+(!!unit?"&nbsp;"+unit:"")):"-";};parCount+=putAGroup(study.parameters,function(p){if(study.effects[0].conditions[p]!==undefined||study.effects[0].conditions[p+" unit"]!==undefined)
