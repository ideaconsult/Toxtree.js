...
substance["IUCFlags"]=flags;ccLib.fillTree(self.rootElement,substance);jT.call(self,substance.referenceSubstance.uri,function(dataset){if(!!dataset){jToxDataset.processDataset(dataset,null,fnDatasetValue);ccLib.fillTree(rootTab,dataset.dataEntry[0]);}});self.querySummary(substance.URI);self.queryComposition(substance.URI);}});}};return cls;})();window.jT=window.jToxKit={templateRoot:null,queries:{taskPoll:"/task/<1>",},templates:{},tools:{},settings:{jsonp:false,crossDomain:false,host:null,timeout:15000,pollDelay:200,onConnect:function(s){},onSuccess:function(s,c,m){},onError:function(s,c,m){if(!!console&&!!console.log)console.log("jToxKit call error ("+c+"): "+m+" from request: ["+s+"]");},},formBaseUrl:function(url){return url.protocol+"://"+url.host+(url.port.length>0?":"+url.port:'')+'/'+url.segments[0]+'/';},init:function(){var self=this;self.$=jQuery.noConflict();self.initTemplates();jT.$(document).on('click','.jtox-toolkit span.ui-icon-copy',function(e){ccLib.copyToClipboard(jT.$(this).data('uuid'));return false;});var url=ccLib.parseURL(document.location);var queryParams=url.params;queryParams.host=self.formBaseUrl(url);self.settings=jT.$.extend(self.settings,queryParams);var initKit=function(element,params){if(params.kit=="study")
