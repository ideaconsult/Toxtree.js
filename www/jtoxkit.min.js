...
if(rlen>0)
$(self.fixTable).dataTable().fnSort([[1,"asc"]]);},"oLanguage":{"sEmptyTable":" - "}}))[0];},updateTables:function(){var self=this;self.filterEntries($('.jtox-ds-control input',self.rootElement).val());},prepareGroups:function(){var self=this;var grps=self.settings.configuration.groups;self.groups={};for(var i in grps){var grp=grps[i];var grpArr=(typeof grp=="function")?grp(i,self.features):grp;self.groups[i]=grpArr;for(var i=0,glen=self.groups;i<glen;++i)
...
self.datasetUri=datasetUri;jToxKit.call(self,ccLib.addParameter(datasetUri,"page=0&pagesize=1"),function(dataset){if(!!dataset){self.features=dataset.feature;cls.processFeatures(self.features);self.prepareGroups();if(self.settings.showTabs){self.prepareTabs($('.jtox-ds-features',self.rootElement)[0],true,function(id,name,parent){var fEl=jToxKit.getTemplate('#jtox-ds-feature');parent.appendChild(fEl);ccLib.fillTree(fEl,{title:name.replace(/_/g,' ')});$(fEl).data('feature-id',id);return fEl;});}
