...
var jToxQuery=(function(){var defaultSettings={scanDom:true,dom:{kit:null,widgets:{},},configuration:{handlers:{query:function(el,query){query.query();},}}};var cls=function(root,settings){var self=this;self.rootElement=root;self.handlers={};jT.$(root).addClass('jtox-toolkit');self.settings=jT.$.extend(true,{},defaultSettings,jT.settings,settings);self.mainKit=null;if(self.settings.scanDom){jT.$('.jtox-toolkit',self.rootElement).each(function(){if(jT.$(this).hasClass('jtox-widget'))
self.settings.dom.widgets[jT.$(this).data('kit')]=this;else
self.settings.dom.kit=this;});}
self.initHandlers(root);if(!ccLib.isNull(self.settings.initialQuery))
setTimeout(function(){self.query();},200);};cls.prototype={addHandlers:function(handlers){self.settings.configuration.handlers=jT.$.extend(self.settings.configuration.handlers,handlers);},element:function(handler){return this.handlers[handler];},widget:function(name){return this.settings.dom.widgets[name];},kit:function(){if(!this.mainKit)
this.mainKit=jT.kit(this.settings.dom.kit);return this.mainKit;},initHandlers:function(root){var self=this;var fireHandler=function(e){var handler=self.settings.configuration.handlers[jT.$(this).data('handler')];if(!!handler)
ccLib.fireCallback(handler,this,this,self);else
console.log("jToxError: referring unknown handler: "+jT.$(this).data('handler'));};jT.$('.jtox-handler',root)
.on('change',fireHandler)
.each(function(){self.handlers[jT.$(this).data('handler')]=this;});jT.$('button.jtox-handler',root).on('click',fireHandler);},query:function(){var uri=this.settings.service||'';for(var w in this.settings.dom.widgets){var widget=jT.kit(this.settings.dom.widgets[w]);if(!widget)
...
if(!!uri)
this.kit().query(uri);}};cls.queryKit=function(element){var query=null;jT.$(element).parents().each(function(){var kit=jT.kit(this);if(!kit)
return;if(kit instanceof jToxQuery)
query=kit;});return query;};return cls;})();var jToxSearch=(function(){var defaultSettings={defaultSmiles:'50-00-0',configuration:{handlers:{onKetcher:function(service,method,async,parameters,onready){if(service=="knocknock")
onready("You are welcome!",null);else
jT.call(null,'molecules/'+service,parameters,function(res,jhr){onready(res,jhr);});}}}};var queries={'auto':"/query/compound/search/all",'similarity':"/query/similarity",'smarts':"/query/smarts"};var cls=function(root,settings){var self=this;self.rootElement=root;jT.$(root).addClass('jtox-toolkit');self.settings=jT.$.extend({},defaultSettings,jT.settings,settings);self.rootElement.appendChild(jT.getTemplate('#jtox-search'));self.queryKit=jToxQuery.queryKit(self.rootElement);self.queryKit.initHandlers(root);self.search={mol:"",type:""};var form=jT.$('form',self.rootElement)[0];form.onsubmit=function(){return false;}
var radios=jT.$('.jq-buttonset',root).buttonset();var onTypeClicked=function(){form.searchbox.placeholder=jT.$(this).data('placeholder');jT.$('.search-pane .dynamic').addClass('hidden');jT.$('.search-pane .'+this.id).removeClass('hidden');self.search.type=this.value;};jT.$('.jq-buttonset input',root).on('change',onTypeClicked);ccLib.fireCallback(onTypeClicked,jT.$('.jq-buttonset input',root)[0]);jT.$(form.searchbox)
...
.on('blur',function(){jT.$(this).css('width','');}).on('change',function(){self.search.mol=null;self.search.type="auto";});var ketcherBox=jT.$('.ketcher',root)[0];var ketcherReady=false;var onKetcher=function(service,method,async,parameters,onready){if(service=="knocknock")
onready("You are welcome!",null);else
...
else{form.searchbox.value=smiles;self.search.mol=mol;self.search.type="mol;"}});jT.$(emptySpace.appendChild(jT.getTemplate('#ketcher-drawbutton'))).on('click',function(){ketcher.setMolecule(form.searchbox.value);});ketcherReady=true;}
};jT.$(form.drawbutton).on('click',function(){if(jT.$(ketcherBox).hasClass('shrinken')){ensureKetcher();jT.$(ketcherBox).css('display','');}
else
setTimeout(function(){jT.$(ketcherBox).css('display','none');},500);setTimeout(function(){jT.$(ketcherBox).toggleClass('shrinken')},100);});};cls.prototype={modifyUri:function(uri){var form=jT.$('form',this.rootElement)[0];var type=jT.$('input[name="searchtype"]:checked',form).val();var res=queries[type]+(uri.indexOf('?')>-1?'':'?')+uri;var params={};if(!!this.search.url){params.type='url';params.search=encodeURI(this.search.url);}
else if(!!this.search.mol){params.b64search=$.base64.encode(this.search.mol);params.type='mol';}
else{params.search=form.searchbox.value;params.type="smiles";if(!params.search)
...
this.setNeedle(needle);this.queryKit.query();},setNeedle:function(needle){if(!!needle)
console.log("jToxError: Trying to set null needle");else{this.search.mol=null;this.search.type="auto";jT.$('form',this.rootElement)[0].searchbox.value=needle;}},getNeedle:function(){return jT.$('form',this.rootElement)[0].searchbox.value;}};return cls;})();var jToxDataset=(function(){var defaultSettings={"showTabs":true,"showExport":true,"showControls":true,"pageSize":20,"pageStart":0,"metricFeature":"http://www.opentox.org/api/1.1#Similarity","fnAccumulate":function(fId,oldVal,newVal,features){if(ccLib.isNull(newVal))
...
return;empty=false;if(idx=="name"){if(isMain)
...
divEl.id=tabId;all.appendChild(divEl);divEl=jT.$('.jtox-exportlist',divEl)[0];for(var i=0,elen=self.settings.configuration.exports.length;i<elen;++i){var expo=self.settings.configuration.exports[i];var el=jT.getTemplate('#jtox-ds-download');divEl.appendChild(el);jT.$('a',el)[0].href=ccLib.addParameter(self.datasetUri,"media="+encodeURIComponent(expo.type));var img=el.getElementsByTagName('img')[0];img.alt=img.title=expo.type;img.src=jT.settings.baseUrl+expo.icon;}}
...
substance["IUCFlags"]=flags;ccLib.fillTree(self.rootElement,substance);jT.call(self,substance.referenceSubstance.uri,function(dataset){if(!!dataset){jToxDataset.processDataset(dataset,null,fnDatasetValue);ccLib.fillTree(rootTab,dataset.dataEntry[0]);}});self.querySummary(substance.URI+"/studysummary");self.queryComposition(substance.URI+"/composition");}});}};return cls;})();window.jT=window.jToxKit={templateRoot:null,queries:{taskPoll:{method:'GET',service:"/task/{id}"},},templates:{},tools:{},settings:{plainText:false,jsonp:false,crossDomain:false,baseUrl:null,fullUrl:null,timeout:15000,pollDelay:200,onConnect:function(s){},onSuccess:function(s,c,m){},onError:function(s,c,m){if(!!console&&!!console.log)console.log("jToxKit call error ("+c+"): "+m+" from request: ["+s+"]");},},formBaseUrl:function(url){return!!url.host?url.protocol+"://"+url.host+(url.port.length>0?":"+url.port:'')+'/'+url.segments[0]+'/':null;},initKit:function(element){var self=this;var dataParams=self.$.extend(true,{},self.settings,self.$(element).data());if(!dataParams.manualInit){var kit=dataParams.kit;var realInit=function(params){if(!kit)
...
jT.$(element).data('jtKit',realInit(dataParams));}},init:function(root){var self=this;if(!root){self.initTemplates();jT.$(document).on('click','.jtox-toolkit span.ui-icon-copy',function(e){ccLib.copyToClipboard(jT.$(this).data('uuid'));return false;});var url=self.settings.fullUrl=ccLib.parseURL(document.location);var queryParams=url.params;if(!queryParams.baseUrl)
queryParams.baseUrl=self.formBaseUrl(url);self.settings=self.$.extend(true,self.settings,queryParams);root=document;}
self.$('.jtox-toolkit',root).each(function(i){self.initKit(this);});},kit:function(element){return $(element).data('jtKit');},initTemplates:function(){var self=this;var root=jT.$('.jtox-template')[0];if(!root){root=document.createElement('div');root.className='jtox-template';document.body.appendChild(root);}
...
catCol={};return catCol;};col=this.$.extend(col,(!!group?getColDef('_'):{}),getColDef(category));return ccLib.isNull(col.bVisible)||col.bVisible?col:null;},sortColDefs:function(colDefs){colDefs.sort(function(a,b){var valA=ccLib.isNull(a.iOrder)?0:a.iOrder;var valB=ccLib.isNull(b.iOrder)?0:b.iOrder;return valA-valB;});},shortenedData:function(data,message,deflen){var res='';if(ccLib.isNull(deflen))
...
return this.settings.baseUrl;},service:function(kit,service,data,callback){var params={};if(!!kit&&kit.queries[service]!==undefined){var info=kit.queries[service];service=info.service;params.method=info.method;params.data=data;}
