...
self.baseUrl=ccLib.isNull(self.settings.baseUrl)?jToxKit.grabBaseUrl(datasetUri):self.settings.baseUrl;self.datasetUri=datasetUri;jToxKit.call(self,ccLib.addParameter(datasetUri,"page=0&pagesize=1"),function(dataset){if(!!dataset){self.features=dataset.feature;cls.processFeatures(self.features);dataset.features=self.features;self.prepareGroups(dataset);if(self.settings.showTabs){self.prepareTabs($('.jtox-ds-features',self.rootElement)[0],true,function(id,name,parent){var fEl=jToxKit.getTemplate('#jtox-ds-feature');parent.appendChild(fEl);ccLib.fillTree(fEl,{title:name.replace(/_/g,' ')});$(fEl).data('feature-id',id);return fEl;});}
...
substance.showname=substance.name;ccLib.fillTree(self.rootElement,substance);jToxKit.call(self,substance.referenceSubstance.uri,function(dataset){if(!!dataset){jToxDataset.processDataset(dataset,null,fnDatasetValue);ccLib.fillTree(rootTab,dataset.dataEntry[0]);}});self.querySummary(substance.URI);self.queryComposition(substance.URI);}});}};return cls;})();window.jToxKit={templateRoot:null,queries:{taskPoll:"/task/<1>",},templates:{},settings:{jsonp:false,crossDomain:false,baseUrl:null,host:null,timeout:15000,pollDelay:200,onConnect:function(s){},onSuccess:function(s,c,m){},onError:function(s,c,m){if(!!console&&!!console.log)console.log("jToxKit call error ("+c+"): "+m+" from request: ["+s+"]");},},formBaseUrl:function(url){return url.protocol+"://"+url.host+(url.port.length>0?":"+url.port:'')+'/'+url.segments[0]+'/';},init:function(){var self=this;self.initTemplates();$(document).on('click','.jtox-toolkit span.ui-icon-copy',function(e){ccLib.copyToClipboard($(this).data('uuid'));return false;});var url=ccLib.parseURL(document.location);var queryParams=url.params;queryParams.host=self.formBaseUrl(url);self.settings=$.extend(self.settings,queryParams);var initKit=function(element,params){if(params.kit=="study")
