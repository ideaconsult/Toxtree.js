...
substance["IUCFlags"]=flags;ccLib.fillTree(self.rootElement,substance);jT.call(self,substance.referenceSubstance.uri,function(dataset){if(!!dataset){jToxDataset.processDataset(dataset,null,fnDatasetValue);ccLib.fillTree(rootTab,dataset.dataEntry[0]);}});self.querySummary(substance.URI+"/studysummary");self.queryComposition(substance.URI+"/composition");}});}};return cls;})();window.jT=window.jToxKit={templateRoot:null,queries:{taskPoll:{method:'GET',service:"/task/{id}"},},templates:{},tools:{},settings:{jsonp:false,crossDomain:false,host:null,timeout:15000,pollDelay:200,onConnect:function(s){},onSuccess:function(s,c,m){},onError:function(s,c,m){if(!!console&&!!console.log)console.log("jToxKit call error ("+c+"): "+m+" from request: ["+s+"]");},},formBaseUrl:function(url){return url.protocol+"://"+url.host+(url.port.length>0?":"+url.port:'')+'/'+url.segments[0]+'/';},init:function(){var self=this;self.initTemplates();jT.$(document).on('click','.jtox-toolkit span.ui-icon-copy',function(e){ccLib.copyToClipboard(jT.$(this).data('uuid'));return false;});var url=ccLib.parseURL(document.location);var queryParams=url.params;queryParams.host=self.formBaseUrl(url);self.settings=jT.$.extend(self.settings,queryParams);var initKit=function(element,params){var kit=params.kit;if(!kit)
...
return this.settings.host;},service:function(kit,service,data,callback){var params={};if(!!kit&&kit.queries[service]!==undefined){var info=kit.queries[service];service=info.service;params.method=info.method;params.data=data;}
this.call(kit,ccLib.formatString(service,data),params,callback);},call:function(kit,service,params,callback){if(typeof params!='object'){callback=params;params={};}
