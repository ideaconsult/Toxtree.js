...
query=kit;});return query;};return cls;})();var jToxSearch=(function(){var defaultSettings={defaultSmiles:'50-00-0',smartsList:'funcgroups',configuration:{handlers:{}}};var queries={'auto':"/query/compound/search/all",'similarity':"/query/similarity",'smarts':"/query/smarts"};var cls=function(root,settings){var self=this;self.rootElement=root;jT.$(root).addClass('jtox-toolkit');self.settings=jT.$.extend({},defaultSettings,jT.settings,settings);self.rootElement.appendChild(jT.getTemplate('#jtox-search'));self.queryKit=jToxQuery.queryKit(self.rootElement);self.queryKit.initHandlers(root);self.search={mol:"",type:""};var form=jT.$('form',self.rootElement)[0];form.onsubmit=function(){return false;}
...
df.appendChild(grp);}
form.smarts.appendChild(df);form.smarts.firstElementChild.checked=true;jT.$(form.smarts).on('change',function(){var hint=jT.$(this[this.selectedIndex]).data('hint');form.smarts.title=(!!hint?hint:'');form.searchbox.value=this.value;});}
...
else{form.searchbox.value=smiles;self.search.mol=mol;self.search.type="mol;"}});jT.$(emptySpace.appendChild(jT.getTemplate('#ketcher-drawbutton'))).on('click',function(){ketcher.setMolecule(form.searchbox.value);});ketcherReady=true;}};jT.$(form.drawbutton).on('click',function(){if(jT.$(ketcherBox).hasClass('shrinken')){ensureKetcher();jT.$(ketcherBox).css('display','');}
else
setTimeout(function(){jT.$(ketcherBox).css('display','none');},500);setTimeout(function(){jT.$(ketcherBox).toggleClass('shrinken')},50);});};cls.prototype={modifyUri:function(uri){var form=jT.$('form',this.rootElement)[0];var type=jT.$('input[name="searchtype"]:checked',form).val();var res=queries[type]+(uri.indexOf('?')>-1?'':'?')+uri;var params={};if(!!this.search.url){params.type='url';params.search=encodeURI(this.search.url);}
