...
self.processComposition(composition);ccLib.fireCallback(self.settings.onComposition,self,composition.composition);});},querySubstance:function(substanceURI){var self=this;self.baseUrl=ccLib.isNull(self.settings.baseUrl)?jT.grabBaseUrl(substanceURI):self.settings.baseUrl;var rootTab=jT.$('.jtox-substance',self.rootElement)[0];jT.call(self,substanceURI,function(substance){if(!!substance&&!!substance.substance&&substance.substance.length>0){substance=substance.substance[0];substance["showname"]=substance.publicname||substance.name;var flags='';for(var i=0,iLen=substance.externalIdentifiers.length;i<iLen;++i){if(i>0)
flags+=', ';flags+=substance.externalIdentifiers[i].id||'';}
substance["IUCFlags"]=flags;ccLib.fillTree(self.rootElement,substance);jT.call(self,substance.referenceSubstance.uri,function(dataset){if(!!dataset){jToxCompound.processDataset(dataset,null,fnDatasetValue);ccLib.fillTree(rootTab,dataset.dataEntry[0]);}});ccLib.fireCallback(self.settings.onLoaded,self,substance.substance);self.querySummary(substance.URI+"/studysummary");self.queryComposition(substance.URI+"/composition");}});}};return cls;})();window.jT=window.jToxKit={templateRoot:null,queries:{taskPoll:{method:'GET',service:"/task/{id}"},},templates:{},tools:{},settings:{plainText:false,jsonp:false,crossDomain:false,baseUrl:null,fullUrl:null,timeout:15000,pollDelay:200,onConnect:function(s){},onSuccess:function(s,c,m){},onError:function(s,c,m){if(!!console&&!!console.log)console.log("jToxKit call error ("+c+"): "+m+" from request: ["+s+"]");},},formBaseUrl:function(url){return!!url.host?url.protocol+"://"+url.host+(url.port.length>0?":"+url.port:'')+'/'+url.segments[0]:null;},initKit:function(element){var self=this;var dataParams=self.$(element).data();if(!dataParams.manualInit){var kit=dataParams.kit;var topSettings=self.$.extend(true,{},self.settings);self.$(self.$(element).parents('.jtox-toolkit').toArray().reverse()).each(function(){if(!self.$(this).hasClass('jtox-widget')){topSettings=self.$.extend(true,topSettings,self.$(this).data());}});dataParams=self.$.extend(true,topSettings,dataParams);var realInit=function(params){if(!kit)
