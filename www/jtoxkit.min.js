...
console.log("jToxQuery: referring unknown handler: "+jT.$(this).data('handler'));};jT.$('.jtox-handler',root)
...
self.setAuto(self.settings.search);}};cls.prototype={modifyUri:function(uri){var form=jT.$('form',this.rootElement)[0];var type=jT.$('input[name="searchtype"]:checked',form).val();var res=queries[type]+(uri.indexOf('?')>-1?'':'?')+uri;var params={type:this.search.type};if(!!this.search.mol){params.b64search=$.base64.encode(this.search.mol);}
else{params.search=form.searchbox.value;if(!params.search)
params.search=this.settings.defaultSmiles;}
if(type=='similarity')
params.threshold=form.threshold.value;else if(type=="auto"&&params.type=='url')
return form.searchbox.value;return ccLib.addParameter(res,$.param(params));},makeQuery:function(needle){if(!!needle)
...
box.value=url;}};return cls;})();var jToxDataset=(function(){var defaultSettings={"showTabs":true,"showExport":true,"showControls":true,"pageSize":20,"pageStart":0,"metricFeature":"http://www.opentox.org/api/1.1#Similarity","fnAccumulate":function(fId,oldVal,newVal,features){if(ccLib.isNull(newVal))
