...
self.processSummary(summary.facet);var hash=jT.settings.fullUrl.hash;if(!!hash){var div=jT.$('.jtox-study-tab.'+hash,self.root)[0];if(!!div){for(var idx=0,cl=div.parentNode.children.length;idx<cl;++idx)
if(div.parentNode.children[idx].id==div.id)
break;$(self.tabs).tabs('option','active',idx-1);}}});},queryComposition:function(compositionURI){var self=this;jT.call(self,compositionURI,function(composition){if(!!composition&&!!composition.composition)
...
substance["IUCFlags"]=flags;ccLib.fillTree(self.rootElement,substance);jT.call(self,substance.referenceSubstance.uri,function(dataset){if(!!dataset){jToxDataset.processDataset(dataset,null,fnDatasetValue);ccLib.fillTree(rootTab,dataset.dataEntry[0]);}});self.querySummary(substance.URI+"/studysummary");self.queryComposition(substance.URI+"/composition");}});}};return cls;})();window.jT=window.jToxKit={templateRoot:null,queries:{taskPoll:{method:'GET',service:"/task/{id}"},},templates:{},tools:{},settings:{jsonp:false,crossDomain:false,baseUrl:null,fullUrl:null,timeout:15000,pollDelay:200,onConnect:function(s){},onSuccess:function(s,c,m){},onError:function(s,c,m){if(!!console&&!!console.log)console.log("jToxKit call error ("+c+"): "+m+" from request: ["+s+"]");},},formBaseUrl:function(url){return!!url.host?url.protocol+"://"+url.host+(url.port.length>0?":"+url.port:'')+'/'+url.segments[0]+'/':null;},initKit:function(element){var self=this;var dataParams=self.$.extend(true,self.settings,self.$(element).data());if(!dataParams.manualInit){var kit=dataParams.kit;var realInit=function(params){if(!kit)
