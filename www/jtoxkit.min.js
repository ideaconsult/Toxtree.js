...
else if(!isMain||level==1){var title=self.feature[fId].title;if(!ccLib.isNull(title)){var fEl=nodeFn(fId,title,divEl);if(isMain&&self.settings.rememberChecks)
...
self.equalizeTables();}:null);var checkList=jT.$('.jtox-ds-features .jtox-checkbox',self.rootElement);var checkIdx=-1;for(var gr in self.groups){ccLib.enumObject(self.groups[gr],function(fId,idx,level){if(idx=="name"){++checkIdx;return;}
...
col["mRender"]=feature.render;if(level==1)
