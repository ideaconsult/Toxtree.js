...
box.value=url;}};return cls;})();var jToxCompound=(function(){var defaultSettings={"showTabs":true,"showExport":true,"showControls":true,"pageSize":20,"pageStart":0,"rememberChecks":false,"metricFeature":"http://www.opentox.org/api/1.1#Similarity","fnAccumulate":function(fId,oldVal,newVal,features){if(ccLib.isNull(newVal))
...
else if(!isMain||level==1){var title=self.features[fId].title;if(!ccLib.isNull(title)){var fEl=nodeFn(fId,title,divEl);if(self.settings.rememberChecks)
jT.$('input[type="checkbox"]',fEl)[0].checked=(self.featureStates[fId]===undefined||self.featureStates[fId]);}}});if(empty)
